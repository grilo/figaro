/* Generated by scripts/build-design-system.mjs. Source hash: a58fd2a99e5f4e4d6e4542f7ea64b6b8f6c3ad3d2a59dd207b5ff3bc68b8cad1. Do not edit directly. */
(() => {
    ...
    for (let prop in spec) {
      ...
      for (let part of prop.split(" "))
        if (part) {
          ...
          if (!inner)
            throw new RangeError("Invalid path: " + part);
          let rule2 = new Rule(tags2, mode, last > 0 ? pieces.slice(0, last) : null);
          byName[inner] = rule2.sort(byName[inner]);
        }
    }
  ...
  // frontend/js/core/writingTechnicalModel.js
  var fileExtensions = new Set("md markdown mdx txt rst org pdf doc docx odt rtf csv tsv xls xlsx ods ppt pptx odp js mjs cjs jsx ts tsx json jsonl yaml yml toml xml html htm css scss sass less svg py go rs c h cpp hpp java kt swift rb php sh bash zsh fish sql vue svelte png jpg jpeg gif webp avif ico bmp tif tiff mp3 mp4 wav ogg webm mov zip gz tar bz2 xz 7z rar exe dll so dylib wasm lock log ini cfg conf env".split(" "));
  ...
  // frontend/js/core/writingContextModel.js
  var wordsPattern = "(?:[ \\t]+[\\p{L}][\\p{L}\u2019'-]*)";
  var forwardDefinition = new RegExp(`^(${wordsPattern}{0,4})[ \\t]+\\(([A-Z]{3,5})s?\\)`, "u");
    ...
    const api = {
      ...
      update(value) {
        if (value.stale) api.invalidate();
          ...
          if (invalidated) list.querySelectorAll("button").forEach((control) => {
            control.disabled = false;
          });
          invalidated = false;
          more.disabled = false;
        ...
        if (value.stale && !current?.decisionsFailed) message = failure ? "Previous results are shown. Some checks could not refresh. Retry analysis." : "Refreshing suggestions. Previous results are shown until checks finish.";
        ...
        if (value.stale) list.querySelectorAll("button").forEach((control) => {
          control.disabled = true;
        });
      }
    };
  ...
  function createWritingLensesView({ id = "writing-lenses", onChange, onRetry, onApplyAll, onManageDictionary, compact = true, onLayout = () => {
  } }) {
    const element = document.createElement("section");
    ...
    function renderCounts() {
      element.querySelectorAll('input[type="checkbox"]').forEach((input) => {
        const badge = input.closest("label").querySelector("[data-count]"), count2 = counts?.[input.value] || 0;
        badge.hidden = !input.checked && !input.indeterminate || !count2;
        badge.textContent = String(count2);
        element.querySelector(`#${id}-${input.value}-count`).textContent = badge.hidden ? "" : `${count2} ${count2 === 1 ? "suggestion" : "suggestions"}.`;
      });
    }
