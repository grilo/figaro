...
beforeEach(() => {
    ...
    window.go.desktop.App.SaveClipboardImage = jest.fn().mockResolvedValue({
        success: true,
        path: 'notes/image1.png',
        markdown: '![Image1](image1.png)',
    });
    window.go.desktop.App.GetFileTree.mockResolvedValue([]);
});
...
describe('clipboard image paste', () => {
    ...
    test('uses Async Clipboard image data when WebKitGTK does not expose a paste File', async () => {
        ...
        try {
            ...
            expect(window.go.desktop.App.SaveClipboardImage).toHaveBeenCalledWith(
                'notes/capture.md',
                'image/png',
                'AQIDBA=='
            );
    ...
    test('does not insert Markdown when persistence fails', async () => {
        window.go.desktop.App.SaveClipboardImage.mockResolvedValueOnce({ success: false, error: 'Disk is full' });
    ...
    test('rejects oversized images before encoding or calling the backend', async () => {
        ...
        expect(window.go.desktop.App.SaveClipboardImage).not.toHaveBeenCalled();
        expect(view.dispatch).not.toHaveBeenCalled();
        expect(errorDialog).toHaveBeenCalledWith(
            'Couldn’t paste image',
