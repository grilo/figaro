...
describe('vault link style workflow', () => {
    beforeEach(() => {
        ...
        window.go.desktop.App.LinkStyleLoad = jest.fn().mockResolvedValue({ style: 'markdown' });
        window.go.desktop.App.ChangeLinkStyle = jest.fn().mockImplementation((style) =>
    ...
    test('loads the persisted preference into the themed Settings combobox', async () => {
        window.go.desktop.App.LinkStyleLoad.mockResolvedValueOnce({ style: 'wikilink' });
    ...
    test('applies a keyboard-selected link style and restores focus to the themed combobox', async () => {
        ...
        expect(window.go.desktop.App.ChangeLinkStyle).toHaveBeenCalledWith('wikilink', false);
    ...
    test('restores the themed combobox after cancelling a pointer selection', async () => {
        ...
        expect(window.go.desktop.App.ChangeLinkStyle).not.toHaveBeenCalled();
    ...
    test('rewrite saves dirty Markdown tabs first and refreshes changed open buffers', async () => {
        ...
        expect(window.go.desktop.App.ChangeLinkStyle).toHaveBeenCalledWith('wikilink', true);
