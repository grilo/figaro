...
func (n *Info) doNLP(blk *Block, seg segmenter, split bool) ([]Block, error) {
	...
	if n.Splitting && split {
		...
		for _, p := range strings.SplitAfter(blk.Text, "\n\n") {
			if err := n.checkpoint(len(blks)); err != nil {
				return nil, err
			}
...
func (n *Info) checkpoint(blocks int) error {
	if n.Context != nil {
	...
	if blocks >= 65536 {
		return errors.New("writing analysis exceeds block limit")
	}
	return nil
}
