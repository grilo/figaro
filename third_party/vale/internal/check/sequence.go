...
func NewSequence(cfg *core.Config, generic baseCheck, path string) (Sequence, error) {
	...
	if rule.Model != "" {
		if _, err := nlp.TaggerFor(rule.Model); err != nil {
			return rule, err
		}
	}
...
func makeTokens(s *Sequence, generic baseCheck) error {
	...
	tokens, ok := generic["tokens"].([]interface{})
	if !ok || len(tokens) > 64 {
		return fmt.Errorf("invalid embedded sequence tokens")
	}
	for _, token := range tokens {
		...
		if tok.Min > 16 || tok.Skip > 16 || tok.Min < 0 || tok.Skip < 0 {
			return fmt.Errorf("embedded sequence expansion exceeds limit")
		}
		reps := tok.Min
		if reps < 1 {
			reps = 1
...
func sequenceMatches(idx int, chk Sequence, target NLPToken, words []tag.Token, cursor int) match {
	var text []string
	...
	for jdx := cursor; jdx < len(words); jdx++ {
		tok := words[jdx]
		if tokensMatch(target, tok) {
			index = jdx
	...
	if ok {
		{
			...
			for _, loc := range s.candidates(txt, tok, len(words)) {
				f.Checkpoint()
				if len(alerts) >= core.MaxAlerts {
					return nil, core.ErrWorkLimit
				}
				// These are all possible violations in `txt`:
				m := sequenceMatches(idx, s, tok, words, cursor)
				if m.index < 0 {
					break
				}
				cursor = m.index + 1
