...
func TestGetApplicationVersionReturnsInjectedBuildMetadata(t *testing.T) {
	app := &App{}
	app.configureApplicationVersion("1.7.0")
...
func TestGetDueTaskSummaryCountsUniqueUnfinishedTasksByLocalDate(t *testing.T) {
	...
	content := "- [ ] Today #todo #urgent\n- [ ] Late #wip\n- [x] Finished #done\n"
	writeTestFile(t, vaultPath, "tasks.md", content)
	app.syncKanbanColumns()

	setTestTaskDueDate(t, app, "tasks.md", 1, today)
	setTestTaskDueDate(t, app, "tasks.md", 2, yesterday)
	setTestTaskDueDate(t, app, "tasks.md", 3, today)
...
func TestSetTaskDueDateUpdatesOnlyMetadata(t *testing.T) {
	app, vaultPath := newTestApp(t)
	defer os.RemoveAll(vaultPath)
	original := "- [ ] Fix bug #todo\nKeep  double  spacing\n"
	writeTestFile(t, vaultPath, "tasks.md", original)
	app.syncKanbanColumns()
	setTestTaskDueDate(t, app, "tasks.md", 1, "2026-08-14")
	board, err := app.GetKanbanBoard()
	if err != nil || len(board["todo"]) != 1 || board["todo"][0].DueDate != "2026-08-14" {
		t.Fatal(board, err)
	}
	if got := readTestFile(t, vaultPath, "tasks.md"); got != original {
		t.Fatalf("Markdown changed: %q", got)
	}
	setTestTaskDueDate(t, app, "tasks.md", 1, "")
	board, err = app.GetKanbanBoard()
	if err != nil || board["todo"][0].DueDate != "" {
		t.Fatal(board, err)
	}
	if got := readTestFile(t, vaultPath, "tasks.md"); got != original {
		t.Fatalf("clear changed Markdown: %q", got)
	}
}
...
func TestEmbedFS_HasNativeBackendModule(t *testing.T) {
	...
	if !strings.Contains(string(data), "window.go?.desktop?.App") {
...
func TestEmbedFS_HasCodeLanguageRegistry(t *testing.T) {
	requireGeneratedFrontendAssets(t)
	// The eager language registry references both modern and legacy parsers, so
	// keep representative modules in the embedded app bundle.
	for _, path := range []string{
...
func TestEmbedFS_HasApplicationAndDesignSystemStyles(t *testing.T) {
	for _, path := range []string{
		"frontend/styles.css",
		"frontend/design-system/style-manifest.json",
		"frontend/design-system/theme-contract.json",
	...
	var manifest struct {
		EagerStylesheets []string `json:"eagerStylesheets"`
	}
	if err := json.Unmarshal(data, &manifest); err != nil {
		t.Fatalf("parse embedded style manifest: %v", err)
	}
...
func TestEmbedFS_FontCSS_UsesPortableRelativePath(t *testing.T) {
	...
	if strings.Contains(content, "/vendor/fonts/") {
		t.Error("inter.css still references obsolete /vendor/fonts/ assets")
	}
	if strings.Contains(content, "url('/vendored/fonts/") {
		t.Error("inter.css uses root-relative font URLs, which break direct-file catalogue loading")
	}
	if !strings.Contains(content, "url('./inter-") {
		t.Error("inter.css missing portable paths relative to its own stylesheet")
	}
}
...
func TestDrag_UsesWailsDraggable(t *testing.T) {
	manifestData, err := os.ReadFile("frontend/design-system/style-manifest.json")
	if err != nil {
		t.Fatalf("cannot read style manifest: %v", err)
	}
	var manifest struct {
		EagerStylesheets []string `json:"eagerStylesheets"`
	}
	if err := json.Unmarshal(manifestData, &manifest); err != nil {
		t.Fatalf("parse style manifest: %v", err)
	}
	var content strings.Builder
	for _, stylesheet := range manifest.EagerStylesheets {
		data, err := os.ReadFile(filepath.Join("frontend", filepath.FromSlash(stylesheet)))
		if err != nil {
			t.Fatalf("cannot read manifest stylesheet %s: %v", stylesheet, err)
		}
		content.Write(data)
		content.WriteByte('\n')
	}
	if !strings.Contains(content.String(), "--wails-draggable: drag") {
		t.Error("application styles must set --wails-draggable: drag on .top-bar for native window drag")
	}
	if !strings.Contains(content.String(), "--wails-draggable: no-drag") {
		t.Error("application styles must set --wails-draggable: no-drag on buttons/inputs to allow clicks")
	}
}
