...
- **Top bar**: sidebar toggle and app title/Home control on the left; a clear draggable center region; a `?` Figaro-help button immediately before Settings, followed by native-style minimize/maximize/close controls on the right. Icon-only sidebar, window, and details-pane controls expose explicit action names to assistive technology. The help button and other action buttons deliberately retain the link-style pointer cursor.
...
- **Status bar** fixed at the bottom as two aligned regions. The application-status region follows the file-tree boundary through its 225–500px expanded width and 44px collapsed rail; it shows live status text ("Ready"), startup progress, the delayed activity spinner, and an optional adjacent action such as deletion **Undo**. In the collapsed rail, full live text remains available to assistive technology and through the shared tooltip, a compact activity mark replaces the visible sentence, startup retains a 20px progress track, and **Undo** remains operable. The remaining buffer-status region shows cursor position, the active file buffer's **Scale** value, reading time estimate (words ÷ 200 wpm, minimum 1 min), word/char count, file encoding, Markdown or source-language/Vim mode, backlink count, and committed **Changes** count. **Scale** resets temporary buffer scaling to the permanent Settings default. Backlink and Changes counts are native buttons: unavailable zero states are disabled, while actionable counts open with Enter/Space. A plain-language **Save to history** action appears only while the active file has a version waiting to be recorded; it safely saves pending text and records only that file, then hides again. The footer keeps its fixed 24px single-row contract at narrow widths, removing extended buffer metadata and then lower-priority reading/word details instead of wrapping. Only the application text is an `aria-live` status; changing buffer telemetry is not announced.
...
- A compact **Quick note** action directly below search, and the application-wide **Ctrl/Cmd+N** shortcut, create and open a collision-safe timestamped Markdown file in the real `Inbox` folder. Its icon remains available in the collapsed rail.
- The remaining space is the **Vault** file tree. It is one page-level Tab stop whose roving focus belongs to the last focused row (falling back to the active document or first visible row), rather than to a generic container. Once inside, Up/Down traverse visible rows without opening them, Home/End jump to the boundaries, Right expands a folder or enters its first child, Left collapses a folder or returns to its parent, Enter activates the focused row, and Space toggles its operation selection without opening it. Every selected row uses the same accent-tinted surface; the active document retains `aria-current="page"` but no separate tree background because the active tab identifies the open buffer. Markdown and files recognised by CodeMirror's language registry open normally. Unsupported/binary assets use normal-opacity semantic or generic file icons, remain keyboard-navigable and selectable, expose Cut, Copy, Rename, Appearance, Pin, Reveal, and Delete, and advertise through the row tooltip that they can be double-clicked into the operating system's default application without replacing the current buffer. Above 400 visible logical rows the tree mounts a moving 160-row window; hierarchy levels, focus, current-document state, selection, context-menu focus restoration, drag behavior, scrollbar range, and distant keyboard activation remain path-based rather than DOM-position-based.
- A fixed tool footer keeps Calendar and Kanban reachable beneath the tree. Calendar expands a monthly panel between the tree and footer without taking ownership of the right pane. Its opening displacement moves upward into place; closing mirrors it downward and delays visual hiding until that transition completes, with the shared reduced-motion duration reducing both directions to a near-instant change. Kanban uses one de-duplicated workspace tab. The open Calendar panel keeps its intended height when the vault tree is long; the tree and overflowing Calendar results scroll independently. Vertical wheel input over the month grid browses one month backward or forward, accumulating small trackpad deltas into a deliberate step; wheel input over the selected-day details retains native result scrolling.
...
- **Figaro theme philosophy**: Figaro Dark and Figaro Light are a matched, dog-inspired pair. Each uses one quiet midnight-fur or ivory-paper navigation surface across the titlebar, file tree, and application-status region, plus one uninterrupted reading surface across the active tab, CodeMirror disclosure gutter, editor, and buffer-status region. Figaro Dark makes that reading plane subtly brighter than its navigation plane so the active buffer remains immediately distinguishable without a structural border. Structural borders and alternate-color strips at the tab/editor and file-tree/editor seams are removed; the sidebar-tools divider and buffer-status separators remain deliberately faint. Collar red is reserved for intentional actions and focus, brass for tags and highlights, and both palettes share semantic success, information, warning, and error roles.
- **Figaro CRT Phosphor**: adapts the MIT-licensed Phosphor Design System's deep green, luminous phosphor, amber, cyan, and red roles using the locally bundled JetBrains Mono stack. Its application-status region continues the terminal-black lower edge of the file-tree palette, while buffer status retains the restrained phosphor treatment. A faint non-interactive vignette and minute perspective/scale transform suggest curved glass without changing component geometry. One soft 14px scan line crosses the screen near the end of a 300-second cycle; reduced-motion preferences suppress that animation while retaining the static theme and vignette.
...
- Pressing Space after any valid standalone hashtag exposes **Add due date…**, **Due today**, and **Due tomorrow**, including on ordinary prose, checkbox tasks, and unsaved custom-column tags. **Add due date…** opens the shared themed calendar at the editor caret. Its month grid is the same presentation as the sidebar Calendar: operating-system locale weekday/weekend rules, blank outside-month cells, Today selected initially when adding, live note-intensity fills, due outlines, accessible activity labels, and the same hover/focus details. Any day remains operable here because the picker is selecting a due date. Either shortcut replaces the triggering whitespace with one portable ` [due YYYY-MM-DD](YYYY-MM-DD.md)` link atomically. A line containing `#done`, a line with an existing semantic due link, CSS color tokens, headings, and excluded syntax contexts remain quiet.
...
| `taskDueDateCompletions` + `hashtagCompletionActivator` | Completion source + ViewPlugin | Suggests saved Kanban hashtags while typing and offers tagged-line due-date actions after Space |
...
- The Calendar control in the fixed left-sidebar footer expands an inline monthly grid with month navigation, due-task results, and linked-note results. Its month label, weekday labels, first weekday, and weekend set follow the operating-system locale through `Intl`; both the current `getWeekInfo()` method and older `weekInfo` property are supported, with the CLDR world convention of Monday-first and Saturday/Sunday weekends as the safe fallback. Figaro does not infer, download, or request access to public holidays.
...
- Ordinary weekdays remain fully legible. Weekend days without notes use a muted neutral surface. A daily note named `YYYY-MM-DD.md` and each distinct Markdown file with a non-due link to that date contribute once to the day's note count; repeated links in one file and semantic Kanban due links do not inflate it. Counts map to five contribution-style levels—1, 2–3, 4–6, 7–9, and 10+—whose full rounded background is mixed from the active theme's success color. That note surface overrides weekend muting.
- One full rounded surface derived from the active theme's accent color represents the effective selection. The first Calendar opening of each app session selects Today and its local month. It then moves to a selected note/link/due day; closing and reopening Calendar in the same session restores that day and month, while a new app launch starts from the new local Today rather than a persisted selection. The day it leaves immediately recovers its underlying weekend or note-intensity surface; selecting Today moves the same treatment back. Selection has no persistent accent border, while the standard focus ring appears only for keyboard focus. A due outline remains visible over the selected surface.
...
- Today and days with notes, ordinary date links, or due items are selectable; other empty days remain non-interactive. The open Calendar keeps a stable height so changing the selection never moves the month grid; at normal window heights its detail allocation keeps a due task and linked note fully visible above the fixed workspace tools, and longer results scroll inside that same region. A long file tree yields the space and keeps its own scrollbar.
...
- The preview uses compact theme-aware typography and cell spacing at the full
  ...
  the raw table.
- The table guide stacks fold, editor, and one-step undoable delete actions.
...
- Click or keyboard-activate the `?` button immediately left of Settings, or press unmodified F1 anywhere, to open **Figaro help**. F1 toggles the same surface without invoking browser help. The closed popup is `hidden` and contributes no controls to the Tab order; opening moves focus to the selected help topic, Tab reaches Close, and Escape, Close, or a second F1 restores focus to the control or editor that invoked it.
...
- **Macros** inventories every Figaro-specific authoring macro currently supported: `@today`, `@tomorrow`, and `@yesterday`; their `[YYYY-MM-DD](YYYY-MM-DD.md)` Calendar-link output/direct syntax; the `#todo`, `#wip`, `#done`, and `#custom-column` Kanban forms; the complete `Text #tag [due YYYY-MM-DD](YYYY-MM-DD.md)` tagged-line syntax; and the **Add due date…**, **Due today**, and **Due tomorrow** actions opened by pressing Space after a valid tag. There is no separate `@due` token.
- **Shortcuts** inventories the supported application and editor keys for help, daily notes, global search, sidebar visibility, buffer switching/closing, saving, Find and Replace navigation, exact plain paste, diagnostics, context menus, file-tree rename/delete, and block folding.
...
- Optional due date: `Follow up #todo [due 2026-08-14](2026-08-14.md)`; checkbox syntax may still precede the text but is not required. Figaro treats the source line as authoritative and derives card, Today, Calendar, and reminder state without a second task database.
- After any valid standalone hashtag, including an unsaved custom tag, Space offers the shared calendar, Today, and Tomorrow actions. Lines containing `#done` and lines that already have a semantic due link do not offer another schedule.
...
| Space after `#tag` | Markdown editor | Offer Add due date…, Due today, and Due tomorrow for the tagged line |
...
| F1 | App | Toggle Figaro help and restore focus to its invoker when closed |
...
- The file tree scrolls independently enough to keep its lower items reachable without clipping. An open `.sidebar-calendar-panel` does not shrink against the tree's content height; it takes a stable capped share above the fixed tool footer, with enough room at normal heights to show the common due-task and linked-note details without clipping, while the tree and longer `.cal-linked-notes` results scroll independently.
...
- **Enter rendered blocks**: Vim-only persistent toggle. It remains disabled until Vim mode is enabled; when active, Normal `j` and `k` enter adjacent rendered fences or reveal an adjacent table's raw source instead of skipping the block. Leading frontmatter has a narrower invariant independent of this setting: `gg` preserves Properties and `k` deliberately reveals its raw YAML. Visual selection through fenced blocks and tables reveals their source regardless of this toggle so Visual mode cannot collapse at a preview boundary.
...
- Every operation-selected row uses one accent-tinted background and inset edge. The active document has only a non-visual `aria-current` marker in the tree; any unsaved file buffer uses a warning dot with assistive “Unsaved changes” text. Keyboard focus remains a separate outline, and Cut adds a compact scissors marker rather than another background.
...
### 28.1 Overview
Hovering over a markdown link shows link information on the shared themed tooltip surface: external links show the URL, while internal file links show the file path and an existence check (✓ Exists / ✗ Not found).
