...
| **Kanban** | `kanban` | One sidebar-owned board workspace; persistent control, hashtag, and dashboard entry points reuse it and may update its focused column without adding a title-bar tab |
...
- **Save failure**: A native write error keeps the latest in-memory buffer dirty and presents a blocking danger dialog even in Pure mode. It names the file and concrete cause, states that the open buffer is preserved, and offers **Retry** using the latest text, **Copy unsaved text**, or **Keep editing**. Auto-Save reports once per continuous failure episode; a successful save clears the episode, while an explicit save can surface it again. A failure waits for an already-open application modal instead of replacing it.
...
- Pressing Space after any valid standalone hashtag exposes **Add due date…**, **Due today**, and **Due tomorrow**, including on ordinary prose, checkbox tasks, and unsaved custom-column tags. **Add due date…** opens the shared themed calendar at the editor caret. Its month grid is the same presentation as the Calendar workspace: operating-system locale weekday/weekend rules, blank outside-month cells, Today selected initially when adding, live note-intensity fills, due outlines, accessible activity labels, and the same hover/focus details. Any day remains operable here because the picker is selecting a due date. Either shortcut replaces the triggering whitespace with one portable ` [due YYYY-MM-DD](YYYY-MM-DD.md)` link atomically. A line containing `#done`, a line with an existing semantic due link, CSS color tokens, headings, and excluded syntax contexts remain quiet.
- Styled distinctly (accent color, pointer cursor on hover, subtle background highlight on hover).
- **Clicking a hashtag** opens or reuses the sidebar-owned Kanban workspace scrolled to the column matching that tag, with a brief highlight animation.
