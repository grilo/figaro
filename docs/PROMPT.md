...
- **Typed block guides and folding**: quiet gutter labels identify top-level Markdown elements, including `h1`–`h6`, `raw`, `list`, `task`, `quote`, `callout`, `code`, `mermaid`, `chart`, `table`, `image`, `math`, `html`, and `rule`. Clicking a non-heading guide folds that block; clicking a heading guide folds everything after it through the last block before the next peer or ancestor, so descendants stay inside their parent. The same guide expands the folded source. Fold state changes only the editor view: saved Markdown, Raw Text Preview, and PDFs are unchanged. Leading frontmatter is a `raw` block and heading-shaped fenced content is never promoted to a heading guide.
