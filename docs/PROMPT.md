...
- A fixed tool footer keeps Calendar and Kanban reachable beneath the tree. Calendar expands a monthly panel between the tree and footer without taking ownership of the right pane. The open panel keeps its intended height when the vault tree is long; the tree and overflowing Calendar results scroll independently.
...
- **Frontmatter / Properties**: a complete leading YAML frontmatter block is rendered as a compact, collapsed Properties card. Its left-edge disclosure arrow rotates 90° when expanded, remains in the same viewport position by reserving a stable editor scrollbar gutter, and collapses the panel from that same control; there is no separate close button. The structured panel offers PDF-layout controls for cover page, contents depth, and a vault-relative print stylesheet; preview commands remain in the file-tree and editor context menus rather than appearing in Properties. A Spellcheck section selects the global fallback, either bundled language, or disables checking for that note. Picker listboxes may extend beyond the card, but their full visible surface remains above the following editor lines: hovering an exposed option keeps the picker focused and expanded, and clicking it updates the property without moving the document selection. Enabling a cover also exposes title, subtitle, author, and date fields. Other YAML remains visible as chips and **Add property** opens the source editor with completion; **Edit YAML** always exposes the original portable frontmatter. Notes without frontmatter get a subtle **Add properties** affordance above the editor; expanding it inserts the YAML skeleton with the first H1 as `title`, the OS username as `author`, today's local date, an empty-string `subtitle`, and the PDF defaults `cover-page: false` and `toc-depth: 0`, then immediately shows the structured panel without moving the cursor into raw YAML. Normal cursor navigation into the frontmatter still reveals its source and moving out collapses it again. Custom PDF CSS is opt-in: **Create starter** proposes `pdf.css` beside the active note, copies the bundled comprehensive example only after confirmation, selects it, refreshes the tree, and opens it. Existing CSS is never overwritten; startup and export never create stylesheets. The panel makes targeted scalar edits only, preserving unrelated YAML and comments. Completion in YAML suggests `title`, `subtitle`, `author`, `date`, `aliases`, `tags`, `description`, `created`, `updated`, `status`, `spellcheck`, `cover-page`, `toc-depth`, and `print-stylesheet`; it also offers status, spellcheck-language, and vault-relative CSS-path values.
...
| `markdownBlockGuidesExtension` | Maps pure typed block-guide plans onto source-preserving CodeMirror fold effects; source-code modes retain the native fold gutter |
...
- Results show a filename, the parent vault path on its own line, first matching line, line number, and additional-match count. Shallow parent paths remain complete; deep paths preserve the root and final three folders around an ellipsis so their distinguishing tail remains visible. The complete path remains in the row's accessible name and tooltip. Summary, path, excerpt, and line/count metadata use the accessible muted-text role in both native Figaro themes. The backend transfers that compact preview and exact count rather than every matching source line. Filename matches rank ahead of body-only matches; otherwise newer files rank first.
...
- Use ↑/↓ to select a result, Enter to open the selected result, or Escape to clear and close search without moving focus out of the search field. The combobox retains DOM focus while `aria-activedescendant` and the matching option's `aria-selected` state expose each keyboard selection. Opening a result positions the editor at its first matching line.
...
- A direct **Delete table** button remains outside cell editing and removes the complete table as one undoable CodeMirror transaction.
...
- When enabled, Vim Normal `j`/`k` stop at an adjacent rendered frontmatter or
  fenced block and reveal its raw Markdown source. Tables stay interactive and
...
- Browser `localStorage` keeps webview-local presentation state (selected date, recent files, search filters, font size, text width, the disabled-by-default editor-breadcrumb preference, and a recovery copy of tabs). A missing or malformed session is ignored safely.
...
3. Initialize CodeMirror, the tab manager, and the editor-breadcrumb coordinator; then load the file tree plus vault-scoped appearance/pin preferences and attach tree handlers. Native debounced vault notifications replace polling; content-only changes do not reload the tree, and Figaro's own saved snapshots do not request an already-known Kanban board again.
...
- **Enter rendered blocks**: Vim-only persistent toggle. It remains disabled until Vim mode is enabled; when active, Normal `j` and `k` enter adjacent rendered Markdown blocks or the first/last interactive table cell instead of skipping the widget. Visual selection through fenced blocks reveals their source regardless of this toggle so Visual mode cannot collapse at a preview boundary.
