...
- Opening or closing Document Outline continuously remeasures CodeMirror only
  ...
  and the Delete table action follow the centered writing-column edges
  throughout the animation, then measurement stops after three stable frames.
  Mermaid's `mermaid`/`editor` stack always remains in the left rail. Delete
  table uses a reserved right lane while at least 360 px remains and moves into
  normal flow above the table below that width, so wrapped text expands the
  reservation rather than overlapping the grid.
...
- A direct **Delete table** button remains outside cell editing, follows the shared side-lane/stacked block-action layout, and removes the complete table as one undoable CodeMirror transaction.
