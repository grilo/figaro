...
- Clicking or pressing Enter on an outline or sticky-hierarchy heading moves focus and the normal CodeMirror cursor to that source heading. A later tab switch, History opening, Raw Text Preview opening, PDF Preview opening, Writing lenses opening, or × close releases the pane without changing note content.
...
- Resize grip at the physical window bottom-right, independent of the buffer-status width: drag to resize, calls `WindowSetSize` via the Go backend. It follows the normal footer reveal behavior and is hidden in Pure mode.
