...
- When enabled, Vim Normal and Visual mode map `j`, `k`, Up, and Down to the
  ...
  behavior. These display-row motions still stop at the first and last
  document boundaries.
...
3. Initialize CodeMirror and the tab manager, load the file tree plus vault-scoped appearance/pin preferences, and attach tree handlers. Native debounced vault notifications replace polling; content-only changes do not reload the tree, and Figaro's own saved snapshots do not request an already-known Kanban board again.
