...
protection, dictionary/writing readiness, Vale cache/process preparation, and
...
| `WritingInitialize()` | — | Prepare the packaged Vale adapter in the background using its verified local cache |
| `WritingAnalyze(id, source)` | Vale JSON string | Analyze the current mapped prose snapshot with fixed local rules |
| `WritingCancel(id)` | — | Cancel or prevent a superseded process request |
