...
- A compact nested-list launcher near the editor's top-right appears for every Markdown note while Document outline is enabled. A note without headings keeps it in place as a guarded, focusable `aria-disabled` approved icon button whose hover/focus tooltip and accessible description explain that the note has no headings; Enter and Space do nothing until adding the first heading enables it. Its heading-tree glyph is deliberately distinct from the top bar's workspace-panel toggle. It sits below every visible sticky-heading row rather than covering that hierarchy. It opens **Document outline**, which lists typed nested H1–H6 headings, ignores frontmatter and fenced-code examples, highlights the current cursor/reading section, and focuses the editor at the selected heading. Explicit launcher activation focuses the current/first heading. The launcher remains visible and highlighted while this pane is open; closing from inside the pane restores launcher focus. Refresh retains a focused heading without taking outside focus, and pane replacement/tab switching never restores stale launcher focus.
...
- **Proofreading — grammar and punctuation** uses pinned `retext-indefinite-article` 5.0.0 for “a/an,” `retext-contractions` 6.0.0 for missing/misplaced apostrophes, plus local checks for spaces before `, ; : ! ?` and consecutive commas. Contraction fixes retain an existing apostrophe style or follow the prevailing prose style; correct contractions never become grammar warnings just for using straight/curly punctuation. Article context must include an adjacent prose word without crossing hidden quotes, code, or block separators. Ellipses and expressive `?!`/`!!` punctuation are not errors under this lens. It is a limited reviewed rule set: the UI explicitly states that subject–verb agreement and contextual homophones are not checked. Article policy preserves reviewed consonant-sounding vowels (“a unicorn,” “a European”) and silent consonants (“an hour”), and withholds uncertain pronunciation/dialect families rather than guessing.
...
- Vale is compiled from adapted, pinned source into the Go backend. Its 27 bundled YAML rules and local sentence model initialize eagerly in memory; no executable extraction, helper process, host configuration, or vault text file is needed. Initialization never gates typing or shutdown.
- Eager local workers run the pinned retext/textlint checks and existing conservative nspell logic; the embedded Go Vale engine runs the selected write-good/proselint/Microsoft rules. The Plain language, Directness, Repetition, Consistency, and Readability check families independently request Vale through their UI groups, including when enabling a lens after cached results that lacked native evidence. The textlint kernel and plain-text parser initialize before worker readiness; an initialization failure is retryable. No engine download or remote analysis occurs at runtime. The application controls Vale configuration and does not load vault/host styles. Dependency pins and the reviewed phrase set are recorded in [WRITING_ENGINE.md](WRITING_ENGINE.md).
- The mapping/configuration signature is version 11 and includes package pins, editorial policy version 4, spelling vocabulary version 1, reviewed terminology/acronym exceptions, and readability thresholds. Harper 2.7.0 was evaluated separately and is not bundled; its default output needs a reviewed rule subset and additional projection-context guards before adoption.
...
- Closed writing panes do not build suggestion cards. Stale-result actions keep
  ...
  become inert immediately on editing and regain interaction with fresh results;
