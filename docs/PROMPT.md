...
### 1.4 Workspace
- Before the first vault tree is available, the workspace shell first reads and applies the saved theme and font configuration, then reveals the themed **Loading vault** notice and explicitly starts native vault discovery. It begins with **Discovering notes…**, then presents the exact `loaded / total` Markdown-note count, a determinate accessible progress bar whose fill occupies the complete track height, and a finalization state. The frontend subscribes before `StartVaultLoad()` and immediately reconciles `GetVaultLoadStatus()`, so every phase is observable without a subscribe/start race. The loading view is removed only after the restored document or Today dashboard has replaced it.
...
- Selected theme persisted to `vault/.config/settings.json`; startup restores
  it to the shell before revealing the loading notice or starting vault work.
