...
- **Windows Spanish dead keys**: WebView2 keeps Spanish tilde (`AltGr+4`), diaeresis, acute, grave, and circumflex keys pending instead of inserting an accent immediately. Their matching letters compose normally (for example `n` → `ñ`, `u` → `ü`, and `a` → `á`/`à`/`â`), Space emits the spacing accent, and Backspace or Escape cancels without editing source; another printable non-combining character receives the spacing accent first. For the grave dead key followed by Space, Figaro accepts the first native backtick from `insertText`, `insertCompositionText`, or legacy `textInput`; it inserts a fallback only if the document remains unchanged through key release, and suppresses or repairs one delayed duplicate. This produces exactly one backtick in Vim Insert mode even when the composition event is non-cancelable.
