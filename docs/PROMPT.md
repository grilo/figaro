...
- Flowcharts additionally expose TB/LR/BT/RL direction, straight/smooth/stepped
  ...
  the active controls below it. Each stable summary row uses an ellipsized identity,
  centered shape summary, and trailing solid color swatch. Pointer activation
  does not translate the row, and list selection preserves the chooser's scroll
  offset across the Style-panel refresh. The editor reuses the approved quiet
