...
- **Heading folds**: each ATX heading from `#` through `######` that owns content has a compact disclosure control in the editor gutter. Folding hides everything after that heading through the last line before the next heading at the same or a higher level, so deeper child headings stay inside their parent section. A collapsed placeholder can be clicked to expand the section. Fold state changes only the editor view: saved Markdown, previews, and PDFs are unchanged. Hash-like lines inside frontmatter or fenced code are never fold controls.
...
- **Windows Spanish dead keys**: WebView2 keeps Spanish tilde (`AltGr+4`), diaeresis, acute, grave, and circumflex keys pending instead of inserting an accent immediately. Their matching letters compose normally (for example `n` → `ñ`, `u` → `ü`, and `a` → `á`/`à`/`â`), Space emits the spacing accent, and Backspace or Escape cancels without editing source; another printable non-combining character receives the spacing accent first. For the grave dead key followed by Space, Figaro accepts the first native backtick or inserts a fallback if no text arrives. It identifies and discards a delayed second copy from CodeMirror's actual contenteditable DOM change, rather than depending on `InputEvent.data`, which WebView2 may omit. This produces exactly one backtick per physical input in Vim Insert mode, including repeated input for triple-backtick fences.
...
- **Editor gutters**: a persistent Settings toggle adds CodeMirror line numbers and active-line gutter highlighting to Markdown and source files. It is disabled by default. Independently, the narrow fold gutter remains available for foldable Markdown headings and source-code regions. Bracket matching, undo/redo history, and autocompletion remain always available.
...
| `markdownHeadingFoldingExtension` | Plans nested, source-preserving ATX heading fold ranges from the Markdown syntax tree |
...
7. Load the persisted line-number, Markdown-diagnostics, spellcheck, and Links style preferences, then load the Auto-Save interval and start its active-tab timer. The Auto-Commit toggle is event-driven: when enabled it records only the file that successfully saved.
