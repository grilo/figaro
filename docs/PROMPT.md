...
![A library project in Figaro Dark with linked research notes and a comparison table](images/figaro-editor.png)
...
Keep the README focused on benefits and short workflow introductions. Put
...
### 1.3 Right Sidebar
- The right sidebar is closed by default and is reserved for **History**, **Document Outline**, **Raw Text Preview**, **PDF Preview**, and **Writing lenses**. Opening one replaces the others. Graph keeps its controls and interaction on the canvas rather than adding a right-pane mode. While closed, the mounted zero-width pane is `aria-hidden` and inert, so its Close control and descendants cannot remain in sequential keyboard focus.
- Clicking or pressing Enter/Space on an actionable committed-history button opens the History view for the active file.
- A compact nested-list launcher near the editor's top-right appears for every Markdown note while Document outline is enabled. A note without headings keeps it in place as a guarded, focusable `aria-disabled` approved icon button whose hover/focus tooltip and accessible description explain that the note has no headings; Enter and Space do nothing until adding the first heading enables it. Its heading-tree glyph is deliberately distinct from the top bar's workspace-panel toggle. It sits below every visible sticky-heading row rather than covering that hierarchy. It opens **Document outline**, which lists typed nested H1–H6 headings, ignores frontmatter and fenced-code examples, highlights the current cursor/reading section, and focuses the editor at the selected heading. Keyboard launcher activation focuses the current/first heading; a mouse click while typing keeps editor focus. The launcher remains visible and highlighted while this pane is open; closing from inside the pane restores launcher focus. Refresh retains a focused heading without taking outside focus, and pane replacement/tab switching never restores stale launcher focus.
- Every mode shares one requested session width, starting at 320px and resizable from 220px up to the space left beside a 400px editor. Switching modes never imposes a different minimum or maximum. The selected launcher uses the approved pressed button state; other launchers remain visible. Each tab remembers its open pane (or closed state), restores it after Settings, planning, or document switches only once its owned buffer is mounted, and rebuilds previews from that buffer. Stale activations cannot restore a pane.

### 1.4 Workspace
- On the first shell frame, the workspace reuses webview-local mirrors of the last confirmed bundled theme, fonts, and normalized sidebar width while vault settings remain authoritative; its application status says **Starting Figaro…**, never the premature **Ready**. After the native bridge connects, the portable session and saved tab size, link style, automation, Vim, line-number, sticky-heading, block-guide, outline, diagnostic and editor-navigation preferences hydrate concurrently behind one barrier. Figaro then restores inactive tabs as metadata and reads and mounts only the selected file. That activation awaits the scheduled CodeMirror document replacement; only then does the editor remain concealed for two more layout frames while CodeMirror settles its real line-number gutter, focus decoration, and restored geometry. It becomes visible and interactive with the saved source and one authoritative preference profile before `StartVaultLoad()` begins. The initial file tree, full-vault search/planning index, and language parsers may continue warming after that reveal. The frontend subscribes before the start request and immediately reconciles `GetVaultLoadStatus()`, so every phase is observable without a subscribe/start race. A compact accessible indicator in the file-tree-aligned application-status region progresses from **Discovering notes…** through the exact `loaded / total` Markdown-note count and finalization; it hides on success and remains visible on an index error. `window._appReady` is set only after the eager work has settled.
...
- **Figaro theme philosophy**: Figaro Dark and Figaro Light are a matched, dog-inspired pair. Each uses one quiet midnight-fur or ivory-paper navigation surface across the titlebar, file tree, and application-status region, plus one uninterrupted reading surface across the active tab, CodeMirror disclosure gutter, editor, and buffer-status region. Figaro Dark makes that reading plane subtly brighter than its navigation plane so the active buffer remains immediately distinguishable without a structural border. A writing-surface border budget removes decorative outlines from inactive rendered code, both collapsed and expanded Properties, its metadata, unused stable-footprint space, rendered-table outer surfaces, Settings groups and controls, Kanban columns/cards, sidebar search/capture surfaces, selected file rows, and the outer/pane/group chrome of the Table, Mermaid, and Chart editor dialogs. Physical boundaries remain for semantic structure such as internal table grids, code gutters, keyboard focus, validation/errors, destructive states, and forced-colors operation; selection keeps independent tonal, typographic, and accessible cues. Structural borders and alternate-color strips at the tab/editor and file-tree/editor seams are removed; the sidebar-tools divider and buffer-status separators remain deliberately faint. Collar red is reserved for intentional actions and focus, brass for tags and highlights, and both palettes share semantic success, information, warning, and error roles.
...
- **Auto-save on switch**: When the user switches away from a dirty file tab, Figaro caches its current content and queues a save. An already reported save-failure episode does not trigger an immediate second attempt through navigation; Auto-Save and explicit Retry remain available. The destination can open immediately; a failed save leaves the source tab recoverable from its cache.
...
- **Cursor memory**: Each file tab continuously remembers its latest cursor/selection. It is restored after switching back or closing a workspace view such as Settings, and is included in the workspace session for restart recovery. Cursor movement only updates memory; the session is written on tab switch, window blur or hiding, and quit (waiting at most 1.5 s so a slow disk cannot hold the window open). Losing the latest cursor after a crash is acceptable; note content never depends on it.
...
- **Activation after close or deletion**: Closing the active tab or deleting its file from the tree activates the most recently used surviving tab. Without a surviving history entry, prefer the first remaining file tab, then the first remaining workspace tab; with no tabs, show Home. Deleting a folder applies the same rule after removing all affected file-backed tabs. The selected tab and editor document must move together so saving and closing act on the surviving file, with no editable orphan of the deleted note.
...
- **Session persistence**: Open tabs, active tab, cursor positions, expanded directories, pinned tabs, active document, and last focused tree row are persisted to a machine-local session record per vault (`<app data>/figaro/sessions/<hash of the vault path>.json`), written by temporary file and rename without a forced flush. A legacy `vault/.config/session.json` is read when this machine has no record yet and is otherwise left untouched. An identical snapshot is never rewritten. The webview also keeps UI-only preferences such as recent files, search filters, Kanban density and flow, the editor-breadcrumb toggle, and a local tab snapshot in `localStorage`; the session record is the authoritative workspace record. Cursor-only and buffer-only tab changes do not rewrite that local tab snapshot. Portable session persistence permits one active write and one replaceable pending snapshot; pending callers settle after the latest snapshot is attempted, including failure reporting.
- **Graph workspace**: The fixed **Graph** side tab selects one deduplicated, session-only, all-notes central workspace; pressing the selected control returns to the previous view while retaining the Graph session. The native projection reads the already-built saved-note index rather than walking the vault again. Nodes carry their vault path, display name, first-folder group, daily-note state, and incoming/outgoing degree. Directed edges include exact Markdown and Wikilink targets, case-insensitive exact paths, and only unambiguous basename targets; missing, ambiguous, duplicate, and self links are omitted. A borderless 224px maximum name/path search matching Search notes, borderless zoom group, and **Orphans** `aria-pressed` choice float together at the canvas top-left; there is no toolbar row, graph filter launcher, group legend, or Graph right-pane mode. Daily notes remain ordinary graph members. Directed arrows are always clear, while deterministic layout uses fixed 45/45 spacing and link pull without exposing tuning controls. A direct note color overrides inheritance; otherwise the closest colored ancestor directory supplies the base, with deeper descendants receiving progressively brighter tints. Unstyled top-level groups consume the same accent palette used by **Customize appearance** before deterministic generated hues extend it. A note-specific valid Lucide icon replaces its generic circle. The canvas surface does not change on hover: hover traces direct links temporarily, plain click pins the same trace until another node or empty canvas is clicked, and double-click or Ctrl/Cmd-click opens that note. Dragging pans; the wheel and `+`/`-` controls zoom; `0` fits; arrow keys pan; `[`/`]` pin a trace; and Enter opens the keyboard-selected note. The existing status bar says **Hover or click to trace links, double-click or ctrl+click node to open the file** and replaces irrelevant buffer telemetry rather than drawing a second footer. An inactive Graph workspace defers graph and appearance refresh work until it is activated. Unsaved editor text does not enter this saved-note projection until the normal save/index update succeeds, and Graph is intentionally not restored across launches.
...
- After a rename name is submitted, status messages distinguish saving open files, checking incoming references, waiting for writing data, executing the rename, and refreshing open files. CSS and other non-Markdown files still receive a reference check; an empty result skips the reference dialog. Refused saves, failed reference previews, and refused native renames clear busy state and report **Rename failed** before showing the error.
...
- **Offline spelling**: the spelling checks in per-document **Proofreading** and **Analysis language** are the controls for enabling spellchecking and choosing its language. Settings manages the vault’s personal dictionary; Properties exposes no spelling controls. English US, English UK, and Spanish use bundled Hunspell dictionaries plus this vault’s accepted personal words. Legacy `spellcheck` and `writing-language` YAML and old global settings remain untouched but have no effect. Unknown-word findings use the shared dotted writing marks, hover Apply/Ignore/Add to dictionary, and the same keyboard/mouse/drag contract as other lenses. Frontmatter (including unfinished YAML), fenced/inline/indented code, URLs, email, explicit link-reference identifiers/definitions, footnote markers (including unresolved ones), and link destinations are excluded using the bundled Markdown parser. Explicit reference labels remain eligible. Defined shortcut/collapsed link and image labels receive advisory hints without Apply because their text also identifies the reference; the resolver protects those keys from all prose fixes. English US/UK accept valid terminal-apostrophe possessives and preserve the exact apostrophe/suffix when correcting a misspelled stem; this English policy is not applied to Spanish. A hyphenated compound is accepted when every component is recognized by the active dictionary. All-caps prose is checked; acronym plurals require a recognized stem. Ordinary capitalized words get case-matched alternatives, while uncertain mixed-case and name-only guesses remain conservative. Equivalent Unicode accents use NFC lookup keys without rewriting source. Shared technical masking protects explicit paths, recognized filename extensions, identifiers, URLs and email; ambiguous slash/dot-separated prose remains eligible. Right-click offers at most five local dictionary-validated alternatives; dictionary membership alone does not establish the intended meaning; ambiguous short typos can have no safe replacement. Applying changes one word with Undo. Disabling Proofreading disables its spelling marks and right-click spelling actions without changing source.
- **Frontmatter / Properties**: a complete leading YAML frontmatter block is rendered as a compact Properties row on the same borderless `--hover-bg` tonal surface, 8px corners, and zero elevation used by rendered code; its metadata has no separate pill surfaces, and keyboard focus restores an explicit accent halo. Its left-edge disclosure arrow rotates 90° when expanded, remains in the same viewport position by reserving a stable editor scrollbar gutter, and collapses the panel from that same control; there is no separate close button. The expanded structured panel persists that same surface, radius, and borderless treatment while individual fields and internal section dividers retain their structural boundaries. Independent boolean fields use the approved theme-aware checkbox with deliberate rest, checked, hover, focus, and disabled states. It offers PDF-layout controls for cover page, contents depth, physical page numbers, and a vault-relative print stylesheet; Raw Text/PDF preview commands remain in the editor context menu, with PDF Preview also available from Properties, and are deliberately absent from the file-tree menu. Picker listboxes may extend beyond the card, but their full visible surface remains above the following editor lines: hovering an exposed option keeps the picker focused and expanded, and clicking it updates the property without moving the document selection. Enabling a cover also exposes title, subtitle, author, and date fields. Other YAML remains visible as quiet inline metadata and **Add property** opens the source editor with completion; **Edit YAML** always exposes the original portable frontmatter. Notes without frontmatter get a subtle **Add properties** affordance above the editor; expanding it inserts the YAML skeleton with the first H1 as `title`, the OS username as `author`, today's local date, an empty-string `subtitle`, and the PDF defaults `cover-page: false`, `toc-depth: 0`, and `page-numbers: false`, then immediately shows the structured panel without moving the cursor into raw YAML. Home/document-start navigation, Vim `gg`, programmatic selection, mouse placement, and drag selection keep Properties rendered; only deliberate upward entry with Arrow Up or Vim `k` reveals raw source automatically, and moving out collapses it again. Custom PDF CSS is opt-in: **Create starter** proposes `pdf.css` beside the active note, copies the bundled comprehensive example only after confirmation, selects it, refreshes the tree, and opens it. Once a stylesheet is selected, **Upgrade copy** proposes a separate `-v2.css` path, copies the current starter, appends every original rule as a final override, selects the new path, and leaves both the source and any existing target untouched. The bundled starter declares `figaro-pdf-starter-version: N` in its leading comment; when the selected stylesheet's leading comment already declares the current version, **Upgrade copy** stays visible but disabled with an explanation, and the backend also refuses the upgrade. Files without that marker, or with an older version, remain upgradable. Startup and export never create stylesheets. The panel makes targeted scalar edits only, preserving unrelated YAML and comments. Completion in YAML suggests `title`, `subtitle`, `author`, `date`, `aliases`, `tags`, `description`, `created`, `updated`, `status`, `cover-page`, `toc-depth`, `page-numbers`, and `print-stylesheet`; it also offers status and vault-relative CSS-path values.
...
- **PDF preview isolation**: The right-pane preview is a fixed sandboxed frame with a validated message bridge, not a parent-controlled `srcdoc` document. The frame owns anchor interception and reports web, vault, fragment, and scroll actions to the application; the application never reads the sandboxed frame DOM. Printable block elements (including display math) carry body-relative source-line ranges, and both panes exchange a continuous source position crossing a shared 30% viewport marker; whole-document progress remains only the fallback for an unmapped region. The editor measures from CodeMirror's document top (excluding content padding) and spreads a multi-line rendered block (table, diagram, math) across its whole line range by pixel fraction. The frame builds a monotonic source-to-pixel breakpoint map from leaf anchors once per layout (render, resize, image or font load), interpolates between and inside blocks, excludes blocks rendered out of source order such as footnotes, and answers each scroll by binary search. Table-merge metadata lines render as blank lines so anchors keep the editor's line numbers. Only the pane the reader is scrolling drives: for 300 ms after one pane moves the other, the follower's own scroll events (CodeMirror height corrections, late image loads) are not sent back unless the reader touches that pane. Programmatic frame reports are explicitly marked and skip the source lookup, so a real reader scroll always wins. A re-render restores the editor's source position rather than a whole-document percentage. During splitter resizing the parent sends `set-scroll-sync-paused`, both scroll directions and frame pointer interaction remain quiet, and 80 ms after release one editor-to-preview alignment restores source-level synchronization. This preserves user `html`/`body` print styling while preventing a clicked link from replacing the preview with an external or filesystem document. See `ARCHITECTURE.md` for the protocol and security rationale.
- **Live preview**: Block formatting markers (`#`, list and quote markers) reveal on selected lines. Inline emphasis, strikethrough and code delimiters reveal when the selection overlaps the delimiter, including its boundary. Links and block previews use their own source ranges to reveal editable Markdown. Bullet points render as styled bullets. Task checkboxes (`- [ ]` / `- [x]`) render as interactive HTML checkboxes that toggle on click. Links render as clickable widgets. Unaffected preview state is retained during ordinary cursor movement, and interactive decorations are limited to the visible editor region so large notes remain responsive. Lists, tasks, blockquotes, ordinary links and reference links cache visible source descriptors; selection changes patch only lines or links whose source visibility changes. List/quote indentation reuses measured prefixes until typography, source or tab-size changes require a refresh. Revealed list and quote markers use the body font measured for their hanging indent; quotes are measured in italic. Writing hints index findings by source range and retain unchanged mounted label plans; cursor movement does not scan offscreen findings. A tab switch removes Markdown presentation before replacing the source, never reconfigures or rescans the outgoing note, and restores only the incoming feature stages its source can use before attaching dormant authoring helpers. With the opt-in Vim rendered-block motion, Normal `j`/Down and `k`/Up deliberately place the cursor inside an adjacent block to reveal its source; the same vertical pairs always extend a Visual selection into adjacent fenced source. GFM tables use the same source-range reveal as Mermaid-style block previews, with no nested cell editor. Live content observers receive the latest typing frame, while word statistics settle after a short typing pause; save and tab-switch snapshots stay immediate.
- **Stable rendered-block footprints**: in the editor only, Mermaid, Vega, and ordinary Vega-Lite fences, ordinary fenced code, multi-line display math, and GFM tables reserve the visual height of their Markdown source, including wrapped rows at the current editor width. Entering or leaving source therefore keeps following text fixed. A Figaro-managed table-backed Vega-Lite chart instead reserves its authored chart height plus chrome: its compact JSON lines stay unwrapped while revealed and the opener receives the remaining placeholder height, so following text stays fixed without allowing JSON wrapping to enlarge the chart. Diagram SVGs and KaTeX display output scale down—but never up—to fit; managed charts fill the available width and remain vertically centered. Code retains normal readable text and scrolls within the reserved height when taller. Native code/table scrollbar presses stay inside their preview and do not move the root caret. Vertical wheel/touch scrolling is preview-first while that preview can move, then chains normally to CodeMirror at its top or bottom; a horizontal-only preview passes vertical input directly to the document. Rendered code uses a tonal, borderless surface with 8px rounded corners and line numbers without a vertical separator; its copy control fades in only while the block is hovered or keyboard-focused. Rendered tables retain their grid because it communicates row/column structure, use a slightly smaller 90% font, a compact 1.4 line height, and reduced surface/cell padding while retaining the full writing-column width; grids that remain larger than the slot scroll inside their single visual surface. Loading and recoverable diagram errors stay inside the same slot. Unchanged wrapped-source measurements survive replacement mounts under a bounded per-editor cache. Prepared diagrams retain their SVG nodes and unique local references, returning before paint when render inputs still match. Prepared Mermaid waits through held-key bursts; active composition defers both renderers. New or invalidated renders wait for typing, text composition, and scrolling to become quiet and an idle slot. Inline math, frontmatter/Properties, links, tasks/checkboxes, and all other inline replacements deliberately retain their existing intrinsic behavior. Successfully loaded images use the separate authored-geometry and source-placeholder contract below rather than joining this generalized policy. A loading image, missing image, or missing-Draw.io Create/Open action is the narrow exception: its themed placeholder stays at one source-line height so revealing the raw image syntax does not move following content. Raw Text Preview keeps its own natural source layout and aligns through source offsets rather than inheriting footprint geometry; PDF Preview and generated PDFs retain their independent layout, with authored image and chart geometry carried through their printable representations.
- **Prepared preview content**: code, math, and table previews retain completed rendered content across source reveal and viewport removal, each with at most 32 entries and 4 MiB estimated source/DOM data per editor. Loaded images retain at most 16 elements and 32 MiB including an estimated four bytes per decoded pixel. New wrappers keep copy, cell selection, image resize, and source positions current. Source/render changes invalidate reuse; tables with embedded images or math errors stay on the fresh path. Image activation generations preserve Draw.io updates and file deletion. Cancelled image mounts return transferred content but discard late loader/inspection output. Content reattachment preserves first-paint geometry; it does not preload the entire document or change diagram scheduling.
...
- **Editor gutters**: a persistent Settings toggle adds cursor-relative CodeMirror line numbers and active-line gutter highlighting to Markdown and source files. The primary cursor line has no label; each other logical line shows its distance above or below that line (`1`, `2`, and so on). Primary logical-line changes or document edits redraw CodeMirror's rendered relative-number rows; horizontal motion on the same logical line leaves their labels intact. Gutter accessibility attributes are synchronized on relevant structural changes and written only when different. The setting is disabled by default. Independently, the enabled-by-default **Block guides and folding** setting controls Markdown's typed helper rail—including unfinished-task Kanban/Calendar actions—and unfolds every folded Markdown range when disabled; source-code regions keep their normal chevron folding. Turning block guides back on immediately restores their lane beside the text. Installed gutter widths are reserved before calculating positions, so adding or removing a gutter does not leave controls misaligned with the centered writing column. Bracket matching, undo/redo history, and autocompletion remain always available.
...
- **Typing continuity**: ordinary prose edits map unchanged heading/list/blockquote positions without refreshing their presentation. Outline and task actions resolve current positions after edits. Buffer revisions capture immutable source immediately and convert to text only for a reader, including saves, open previews, paused word-count updates, and planning views. A hidden Kanban does no typing-triggered buffer scan; warm activation synchronously projects the latest unsaved tasks before displaying the existing board. Structural edits, parser progress, actual viewport movement, and typography/configuration changes retain conservative refreshes.
- **Editor update ownership**: remembered cursors remain synchronous in a store keyed by tab ID; buffer revisions remain in immutable tab records. Cursor updates publish one record without copying open tabs, and explicit session snapshots read the live store. Both have separate notifications from tab presentation. Cursor changes do not wake window-title, breadcrumb, navigation, metadata-storage or tab-rendering observers. Document observers declare their dependencies; writing and activity do not receive selection-only updates. Cursor restoration and delayed portable-session persistence remain unchanged.
- **Navigation work reuse**: image, table, math, diagram and code fields reuse parsed source during selection-only motion and patch only blocks whose reveal state changes. Proven prose edits inside unchanged parsed paragraphs, including formatted text, lists and blockquotes, map cached code, image, table and guide positions and existing decorations. Completed paragraphs can reuse previews within equally advanced partial trees. A separate parsed-block check scopes heading/code/table edits and safe soft Enter/Backspace to affected blocks. Uncertain syntax, changed surrounding structure and parser progress use normal invalidation. Unrelated settings preserve cached block projections; reveal-policy, fold and drag changes refresh them. Code/table clicks resolve the mounted decoration's current source position even after scrolling remounts a preview. Unmoved descriptors keep their reveal index. Helper-rail widths are cached per guide set, with indexed widget/viewport lookup. Pure phrase ranges normalize once per document/block and use binary lookup for ordered segments. Known single-tab buffer transitions classify only that record and preserve cursor lifetimes; structural tab publications still reconcile them. Block-guide boundaries and sticky-heading ancestry share a linear-time heading structure index independently of viewport markers. Ordinary single-line prose edits map Outline heading positions and retain unchanged rows; structural, multiline, and Setext-adjacent edits use the full heading parser. Breadcrumbs, pane controls, Pure accessibility state, and the native window title change only with their displayed state. Wrapping rulers batch mounting and height reads before footprint writes, skip plain prose and authored chart heights, and remain before paint; explicit typography changes invalidate cached wrapped heights.
- **Find and Replace**: Ctrl/Cmd+F opens CodeMirror's native search workflow in a fixed 104px three-band panel. The first band holds the query plus Previous, Next, and All; the second holds Match case, Regexp, and By word; the third holds the replacement plus Replace and Replace all. The controls use theme tokens, preserve their native labels and keyboard behavior, remain non-overlapping at narrow widths, and close with Escape. Its accessible match announcement reuses native query matches for the same document, query and word-character context and locates the active result by binary search; edits, query changes and state-dependent custom predicates retain invalidation.
...
### 5.1 Core Extensions
`history()`, `bracketMatching()`, and `autocompletion()` are always installed. `relativeLineNumbers()` plus `highlightActiveLineGutter()` live in a compartment controlled by the persistent, off-by-default **Show line numbers** setting. The relative-number gutter caches the primary cursor's logical line for each editor state and asks CodeMirror to repaint rendered gutter rows only after the primary logical line or document changes. The local Markdown linter uses its own persistent, on-by-default compartment and coordinates conservative synchronous Markdown checks with the shared asynchronous Mermaid parser; spelling runs through the selected document lens and shares the eagerly registered inline writing extension. Deselecting Spelling removes its findings without changing source or other lenses. `lineWrapping` and live-preview extensions are installed only for Markdown. The root editor's vertical-motion adapter classifies keyboard vertical motions without intercepting them, requests a keyed CodeMirror measure, then corrects the physical scroller from the selected cursor rectangle after paint and requests a final measure. This keeps long notes with rendered blocks synchronized during reversed Arrow Up/Down, Page Up/Page Down, and Vim `j`/`k` motion. Repeated requests share one frame; the final reconciliation does not repeat the same explicit coordinate read. Cursor-only tab updates do not notify file-tree presentation observers; dirty-path changes still refresh markers. Outline reuses headings for the same immutable document. Within a section it performs no heading-row updates; crossing sections changes only the old and new active rows. Images, tables, diagrams, and math use indexed old/new selection overlaps to check reveal changes, keeping unchanged decorations in place. Inclusive range boundaries and multiple selections retain existing reveal behavior; math follows the primary selection head. Pure focus reuses decorations inside an unchanged focus range and segments only the current block; geometry reacts separately to resize/settings changes. Diagram navigation reuses parsed fences and keeps source placeholders mounted while the reveal state is unchanged. Edits, folding, Find, pointer/range selection, and document changes retain their normal invalidation. The file-mode folding compartment installs `markdownBlockGuidesExtension` for Markdown when its enabled-by-default preference is on, installs CodeMirror's normal `foldGutter()` and keymap for recognised source files, and stays empty for plain text.
...
- A custom column emptied while the board is open (by moving, completing or untagging its last card) stays visible and usable in its position until the board is left; the next opening shows only columns whose hashtag is in use. A column emptied while the board is closed is not shown. The three system columns always remain.
...
### 6.2 Task Discovery
- The initial shared vault index normally reads each `.md` file once, deriving tags, cards, dates, and backlinks in one line-oriented walk; known concurrent edits are reconciled before publication, while unknown subtree changes restart discovery. Board reads use its precomputed cards.
...
- **Board area**: a horizontally scrollable row of borderless, tonal columns by default; Settings can switch it to a vertically scrolling stacked flow. Cards use a second tonal layer, lift on pointer hover, and reserve outlines for keyboard focus, transient navigation/drag targets, and forced-colors mode. Active scrolling temporarily suspends hover elevation and its transition while preserving keyboard-focus paint, so cards passing beneath a stationary pointer do not leave trails.
...
- Each card is one labelled keyboard stop. Its header shows cleaned task text and a top-right theme-accented ellipsis; it does not repeat the source filename. Its footer always shows two clickable calendar pills: **Start …** or **Not started** at the left and **Due …** or **No due date** at the right. Due today uses the warning treatment, overdue uses danger, and later dates stay quiet. The ellipsis menu can clear both dates or remove the card's tag. Card-level S/D/Delete and Shift+F10/Menu expose those actions without inserting nested controls between cards in the Tab sequence.
...
- The first board request synchronously shows a theme-aware three-column shimmer skeleton before awaiting indexed cards. Its shared skeleton surface becomes static when reduced motion is requested; feature-owned column and card dimensions preserve the expected board geometry. A loaded workspace remains mounted while inactive, so returning exposes the existing populated surface on its first frame instead of replaying the skeleton or panel entrance. Reprojection keeps the board's horizontal position and each mounted column's vertical reading position.
- A column above 120 cards mounts a moving 96-card window. Its complete logical order remains available to Tab/Shift+Tab, arrows, reordering, cross-column moves, drag/drop, counts, scrollbar navigation, and focus restoration. The DOM adapter measures rendered card strides, maps scroll offsets through the pure height index, and patches only entering/leaving edges; overlapping cards retain identity and the logical viewport coordinate remains stable in every intermediate frame.
...
- **Set color**: 12-color palette picker + "no color" option; persisted to `vault/.config/kanban-colors.json` by hashtag. A color outlives its column: emptying a column or restarting keeps it, so reusing the hashtag later brings the color back. Only choosing "no color", renaming (which moves it) or deleting the column removes it; an emptied, still-visible column can also be colored, renamed or deleted. Choosing a color replaces the header's neutral palette icon with that color's swatch, while choosing "no color" restores the neutral icon.
...
- **Click either date pill** to open its shared calendar picker. Changing Start preserves Due; changing Due preserves Start. **Click the top-right theme-accented ellipsis**, or press **Shift+F10/Menu** on a focused card, to open one menu containing **Clear start and due dates** and **Remove from board**. The former clears only private schedule metadata; the latter strips that specific `#tag` from the line. Pressing **S**, **D**, and **Delete** invokes start-date, due-date, and remove-tag actions directly. Escape returns focus to the card without a write. Repeated date shortcuts and modified shortcuts do not mutate tasks; only Delete removes a tag. Dirty board sources must be saved before scheduling.
...
- An empty Board uses its instruction area to explain column-tag task creation with a `Plan trip #todo` example; populated boards show card shortcuts instead, including after live updates. Loading does not claim there are no tasks, and Gantt hides the Board instruction.
...
- Versioned `.config/task-schedules.json` contains IDs and date ranges separate from the Markdown. The file is eligible for explicit Git tracking; automatic note history does not commit configuration. Board/Gantt scheduling refuses dirty/stale tasks, performs a root-scoped atomic private write, and never inserts dates or IDs into notes. Editor `@date` and checklist Calendar actions explicitly author ordinary date links before scheduling. Malformed metadata or failed writes remain intact and surface an error; the Board still works.
...
- No legacy due-link fallback or migration exists. Clearing End clears the deadline everywhere, while keeping Start unless explicitly cleared. Board cards always show separate Start and Due pills; their combined action menu clears both dates or removes the board tag. The Gantt DOM, picker, pointer state, and task projection are released when another workspace opens.
...
- Calendar Month/Timeline and Kanban Board/Gantt choices share a 24px left and 14px top inset with Graph's floating controls. They use the same canonical segmented-choice primitive as Settings and every other fixed short choice: a pill-shaped track derived from the former Settings control surface, with one separately layered theme-accented pill that slides beneath the selected option. One through four choices share equal-width hit targets; forced-colors mode paints the selected button directly, reduced motion moves the highlight immediately, and keyboard focus retains an explicit outline. Figaro Dark keeps its red selected treatment and Figaro Light keeps its stronger warm control layer.
- Gantt paints the current day once as a continuous one-pixel line from the day heading through the full viewport-height track, including rows and empty space below the last task; day headings and rows do not add duplicate marker edges.
...
- The Calendar control in the fixed left-sidebar footer selects a central monthly workspace with month navigation, due-task results, and linked-note results; reselecting it returns to the previously active view without discarding the Calendar session. The main surface is a borderless 50/50 split: the complete month unit is centered on both axes in the left pane, while the independently scrolling selected-day results occupy the right pane. Calendar preserves the shared footer's 24px layout row and file-tree-aligned application status, but hides the main-pane buffer telemetry as non-applicable. Its month label, weekday labels, first weekday, and weekend set follow the operating-system locale through `Intl`; both the current `getWeekInfo()` method and older `weekInfo` property are supported, with the CLDR world convention of Monday-first and Saturday/Sunday weekends as the safe fallback. Figaro does not infer, download, or request access to public holidays.
- **Timeline** is an alternative presentation inside that same workspace, not another tab. Month remains the launch default; the choice lasts only for the current app session. Timeline materializes 42 consecutive local dates in a centered six-week window while the backend returns only populated dates from the shared Calendar index. The initial anchor is the selected Calendar date or Today. The ordinary outlined **Today** action resets it and Earlier/Later page it by 14 days. Either wheel axis maps onto the horizontal track and advances by at least three measured day-column widths per event; larger deltas retain their magnitude. A focused track supports Left/Right by one measured day plus Home/End. Pressing the primary pointer on empty Timeline space enters a grab state, captures movement into a bounded horizontal pan, suppresses native text selection, and returns to rest on release/cancel; note pills remain ordinary click targets and never arm panning. When the viewport enters the outer 14 loaded day-columns on either side, Timeline silently shifts the anchor by seven days without replacing the current track with a loading surface, then restores the first visible shared date to the same viewport coordinate. Each day follows the operating-system locale for weekday, day, and month labels; Today retains the accent selection treatment. Ordinary columns and the track share the main workspace surface. Timeline reserves its former subtle track color for the same locale-defined weekend set supplied through `Intl.Locale.getWeekInfo()` or its legacy `weekInfo` property, including Friday/Saturday regions and non-contiguous weekend sets. `Intl` does not expose public-holiday dates, so Timeline does not label or guess national or religious holidays.
...
- Collapsing the sidebar keeps the selected Calendar workspace open and leaves its connected control in the 44px rail. History, Document outline, Raw Text Preview, PDF Preview, and Writing lenses share the right pane.
...
### 7.5 Today Dashboard
- The un-tabbed workspace overview is headed **Today** and uses the operating system's local calendar date. **Open/Create today’s note** and Ctrl/Cmd+Shift+N share one in-flight operation that prefers an existing `Inbox/YYYY-MM-DD.md`, then an existing legacy root `YYYY-MM-DD.md`; otherwise it creates the real `Inbox` directory when needed, creates the dated note there once with a date heading, refreshes the tree, and opens it. A concurrent same-name file is treated as the existing daily note and is never overwritten. Directory or note creation errors remain inline on Home with focus returned to the action.
...
- Footnote markers (`[^identifier]`) are excluded from all prose and spelling checks whether defined or unresolved. Definition bodies and inline footnote text (`^[prose]`) remain eligible under the normal writing rules. Escaped literal brackets remain prose. This protection does not add missing-definition diagnostics or change source, rendering, or export.
- **Corpus safety policy:** Spelling recognizes a reviewed English technical vocabulary and acronym plurals. Parsed emphasis delimiters are excluded before identifier masking; closing single quotes and numeric compounds never become partial correction ranges. Valid possessives retain their authored suffixes. Dictionary alternatives cannot invent possessive apostrophes; separately reviewed contraction shapes remain available. Spelling bulk actions require an explicit reviewed-correction flag in addition to one candidate and existing snapshot/source guards. Contextual wordiness guards preserve established technical nouns and postal addresses while retaining useful verb/phrase advice. Directness reader-assumption checks distinguish instructions from easy-read terminology, negation, limiting phrases, titles and descriptive clarity. Acronym recognition supports expansion-before/after-acronym, plural forms, soft wraps and a limited noun-of-noun reorder, without inferring meanings or crossing protected regions. URLs keep balanced internal parentheses and leave surrounding punctuation visible. These pure policies execute in existing background workers and do not add input-handler scans. The detailed regression contract and evaluation limits are in [WRITING_CORPUS_FIXES.md](WRITING_CORPUS_FIXES.md).
- **Quality context:** Recognition-only English vocabulary includes async, dotfiles, etags, fallbacks, middleware, npm, backpressure, fallthrough and pathnames. Lower-camel-case identifiers stay protected. Acronym definitions in spelling-eligible prose suppress spelling alarms for that note only; code definitions do not count and the result never enters the shared dictionary cache. Generated English candidates cannot simply drop a terminal plural `s`. Shared advice guards preserve technical options/limits/forwarding and web addresses, “look forward to,” adjectival “unexpected,” agentless “disappointed,” and literal end-of-day instructions. A visible nesting pass verifies balanced opening marks within one block, never across protected text or paragraph boundaries. Equivalent “there is/are” wordiness and indirect-opening observations share one canonical finding with both sources and independently selectable lenses; old occurrence Ignore kinds remain effective, including partial-provider results. Grammar policy 7 broadens 29 existing families through bounded complements, intervening modifiers, noun-subject questions, and software/info countability; it retains literal, dialect and compound-noun guards. The [quality report](benchmarks/writing-quality-2026-09-20.md) separates development regressions from a sample frozen before tuning.
- A fourth compact launcher beneath Outline, Raw Markdown, and PDF toggles the shared right pane. **Ctrl/Cmd+Shift+L** opens the same surface for an owned Markdown buffer. Opening replaces the current pane; closing preserves choices. Non-Markdown/stale buffers cannot open it.
- **Lenses** offers five equal checkboxes: **Proofreading** (Spelling, Repetition, Consistency, Grammar & punctuation), **Clarity** (Plain language, Readability), **Directness**, **Inclusive language**, and **Formulaic writing**. Any combination, including none, is valid. Configured documents initially collapse these controls behind the shared **Lenses** disclosure with a trailing selected count, while unconfigured notes and the Pure picker expose setup. The count represents active groups, including partial groups. Each selected group also shows a muted `.ui-badge` with its current number of visible findings, counted once under the lens that displays each finding; the number is `aria-hidden` and the checkbox description announces “N suggestions.” so the checkbox name stays the lens label. Counts keep their previous value while retained results refresh and clear when analysis is unavailable. Group IDs (`proofreading`, `clarity`, `directness`, `inclusive-language`, `formulaic-writing`) are distinct from saved check IDs. The approved `.ui-disclosure` keeps a visible neutral resting surface, left chevron and 180 ms in-flow grid reveal; hover/pressed/focus states remain theme-relative in Dark, CRT and other themes. Enter/Space activate its native button. Closed content becomes inert and hidden from assistive technology immediately while its visual collapse finishes; no hidden choices enter Tab navigation. Reduced motion removes both the reveal and chevron transitions. Opening retains focus on the header; a programmatic collapse returns descendant focus to the header before hiding content. Rapid reversals do not use timers. Background preference updates keep the arrow DOM and current expansion, and a different document applies its configured/unconfigured default without animation. Each lens shows one short sentence and an independent, labelled info button beside its name, outside the checkbox label. Clicking the info button opens persistent nonmodal help using the existing menu and icon-button primitives; hovering it shows only “About [lens]”. The help explains coverage and limitations and shows three or four illustrative Before/After examples for supported checks, with no editing actions. Partial groups identify which checks are enabled and label examples as full-lens illustrations. Spanish Proofreading shows only Spanish spelling examples; unsupported lenses omit examples and explain why they cannot be activated. Info remains usable while checkboxes are disabled. The help portals outside clipped ancestors, prefers the editor side when it fits, and falls back above/below with bounded scrolling. Repeated scrolling inside help preserves its outer size; repositioning from content, ancestor scrolling, or window changes uses border-box dimensions including borders and scrollbar chrome, never shrinking cumulatively. Opening focuses Close; Escape or Close returns focus to the invoker, Tab/Shift+Tab resumes the row’s native focus order, and outside pointer/focus dismisses without stealing focus. Help consumes the first Escape in Pure without closing its parent picker. Only one lens help is open per view; document switches, owner closure, disclosure collapse and entering Pure release outgoing help. Live preference updates refresh content while preserving its Close button and focus. Existing subsets keep their exact saved check IDs: a mixed checkbox and visible **Partial** badge identify a partially enabled group, and its help and accessible checkbox description list enabled checks. Selecting a partial group enables all supported members; unchecking a fully selected group clears all its members. No rule, saved Ignore, dictionary word, or analyzer identity is removed by consolidation. There is no primary lens, redundant note-name heading, or writing profile. Selecting Directness enables advisory passive-construction checks directly, with bounded guards for reviewed technical process descriptions and meaningful modifiers; explicit actors remain reviewable.
- **Proofreading — consistency** checks mixed `email/e-mail`, `website/web site`, `online/on-line`, `offline/off-line`, and `ebook/e-book` forms, ignoring ordinary sentence capitalization. It also checks exact mixed `PDF/pdf` and `HTML/html` capitalization. Both authored forms receive a finding with the alternative already used in the document; there is no imposed preferred spelling or bulk replacement. Identical term forms, quoted wording, code, destinations, and recognized identifiers do not create a term-consistency finding. Quotation punctuation belongs to **Formulaic writing**: `retext-quotes` 6.0.2 follows the prevailing straight/curly style of balanced outer quotations, with first occurrence breaking ties and the first quote of that style determining outer single/double nesting. Apostrophes independently follow the prevailing authored prose style, falling back to the quote convention. A single guarded edit changes only validated paired/nested delimiters and preserves the exact enclosed Markdown. Encoded/escaped delimiters remain advisory.
- **Proofreading — consistency** additionally uses `retext-sentence-spacing` 6.0.0 to suggest one space between same-line sentences and `retext-diacritics` 5.0.0 for optional accents in selected names and borrowed words. Spacing observations expand to adjacent words so the underline is usable; examples explicitly state the space counts. No gap crosses masked text or prose regions, and authored soft/hard line breaks remain unchanged. Both packages require safe contiguous mappings for Apply; encoded or discontinuous source remains advisory. Accented alternatives are contextual, preserve capitalization, and never apply automatically.
- Long-note analysis memoizes unchanged Slopless tokenization through a bounded per-analysis cache (4,096 entries, 2 MiB estimated weight), returning independent token copies and clearing on completion/failure. The pinned punctuation rule receives paragraph-local coordinates and reports against the original paragraph, avoiding document-prefix padding while retaining its exact relative findings. All 29 Slopless rules and other package coverage remain available. Exact paragraph-local retext/textlint results are reused across edits in worker caches capped at 8,192 entries and 4 MiB each. The worker retains one note’s unchanged Markdown block maps, reparses safe paragraph/heading edits locally, and rebases UTF-16 positions. Structural changes parse the complete note; reference-context changes invalidate retained maps. Document-wide policy is recomputed from the complete current projection. Full-scan equivalence tests cover edits, paragraph moves/splits, protected Markdown and changing punctuation conventions.
- **Proofreading — grammar and punctuation** uses pinned `retext-indefinite-article` 5.0.0 for “a/an,” `retext-contractions` 6.0.0 for missing/misplaced apostrophes, plus local checks for spaces before `, ; : ! ?` and consecutive commas. Contraction fixes retain an existing apostrophe style or follow the prevailing prose style; correct contractions never become grammar warnings just for using straight/curly punctuation. Article context must include an adjacent prose word without crossing hidden quotes, code, or block separators. Ellipses and expressive `?!`/`!!` punctuation are not errors under this lens. It is a limited reviewed rule set: the UI explains that agreement checks cover selected pronoun patterns and selected contextual homophones; complex subjects and ambiguous meanings still need review. Article policy preserves reviewed consonant-sounding vowels (“a unicorn,” “a European”) and silent consonants (“an hour”), and withholds uncertain pronunciation/dialect families rather than guessing.
- **Clarity — readability** uses the existing parser’s sentence boundaries for paragraph sentence-length advice above 30 words and pinned `retext-readability` 8.0.0 for complex prose. Formula options are `age: 16`, `minWords: 15`, and `threshold: 5/7`; at least five formulas must agree before advising on a sentence of at least 15 words. Both checks omit headings, table cells, and sentences spanning excluded content. Length and formula estimates are separate concerns even on the same sentence; equivalent length observations merge with their sources retained. The pinned Microsoft sentence-length rule uses the same threshold; its anchor expands only to an eligible mapped sentence whose own verified word count exceeds 30. The local check retains coverage when the native tokenizer crosses a Markdown block boundary. Explanations distinguish measured length from formula estimates, which do not measure writing quality. Labelled general examples suggest splitting or simpler wording, with no Apply action, generated rewrite, or reading-grade score. Markdown formatting may split the projected text from the exact source; underline freshness uses the captured source slice while analysis retains the prose text.
- **Clarity — plain language** reviews all native simplification matches as contextual advice; only the seven reviewed phrase forms offer automatic simplification edits. It also uses redundant-acronym fixes and broader cliché, jargon, archaic-wording, ambiguity, and word-choice advice. **Directness** adds optional opening, modifier, qualifying-phrase, and reader-assumption review. Difficulty words (easy/easily/simple/simply/just/basically/straightforward) are reviewed only in sentences that address the reader (you/your/let’s) or begin with a reviewed instruction verb; knowledge words (obvious/obviously/clearly/of course/everyone knows) are reviewed in any sentence; capitalized tone words inside titles are not. Reader-assumption findings use kind `style.reader-assumption` and still honor Ignore decisions saved under `language.inclusive`. Emphatic punctuation (“!!”) is **Formulaic writing** advice. Seven write-good, fourteen proselint, and six Microsoft rules are bundled; their complete included/excluded inventory and contextual guidance contract are in [WRITING_PACKAGE_REVIEW.md](WRITING_PACKAGE_REVIEW.md). These style rules never expose native replacement actions directly; the separate grammar allowlist validates its reviewed literal replacements.
- **Inclusive language** (`inclusive`) is independently selectable and English-only, using `retext-equality` 7.1.0. Only reviewed generic roles/expressions produce individual alternatives: chair titles, mankind, manpower, man-made, blacklist, and whitelist. Reader-assumption tone words belong to Directness. Personal pronouns, neutral identity labels, diagnoses, and other unreviewed upstream equality suggestions are withheld. An exact safe source mapping is necessary but not sufficient for Apply. No inclusive checks are selected automatically, and there is no bulk replacement or global style-rule ignore.
- **Analysis language** is the first control, using the existing Settings-style select-only combobox, and belongs to this document: **None** disables checking, **English US** (`en-US`) and **English UK** (`en-GB`) support all selected checks, and **Spanish** (`es`) supports only the spelling checks in Proofreading. Selecting a language clears unsupported check IDs. A group with no supported checks is unchecked and disabled; Proofreading stays available for Spanish and its description explicitly says only spelling is available. Hovering a disabled checkbox or its label shows the shared themed tooltip with its specific reason: choose an analysis language, use a supported English variant, wait for preferences to load or apply, or retry a failed load. The input exposes the same accessible description. Reasons update with the current state and disappear when the lens becomes available; clicking never enables an unavailable lens. Switching back leaves cleared checks off until explicitly selected again; Proofreading becomes Partial when only its spelling checks remain selected in English. None unchecks and disables all lens checkboxes. Older saved unsupported choices are likewise shown unchecked on load, without writing preferences until an explicit change. Only the existing None, English US/UK, and Spanish options are offered. Legacy frontmatter no longer overrides language or enables/disables spelling. New notes start with no lenses and language None.
- Version 3 `.config/writing-lenses.json` retains the nine internal check IDs for lossless document/default compatibility; the five UI groups expand to supported IDs only on explicit selection. It stores document choices by path: `{"version":3,"preferences":{"lenses":[],"language":"none"},"documents":{"Memo.md":{"preferences":{"lenses":["spelling","direct"],"language":"en-US"}}}}`. Root preferences are fallback defaults for unsaved document choices. The **Apply to all documents** button immediately follows **Choices saved for this document.** and atomically copies the combination and language to defaults and every saved document entry, retaining extension metadata. The controller disables changes while applying, drains earlier per-document saves before the bulk write, updates cached notes after success, and offers retry on failure. No Markdown is changed. Versions 1/2 migrate primary plus overlays to equal lenses and retire the profile only on explicit save; read never writes. Rooted atomic writes preserve unknown fields and other documents, reject corrupt/newer schemas, and never commit. Saves are serialized/coalesced per document; switching notes does not move pending loads/saves to another note. Save errors keep choices and offer **Retry**, including after returning to that note. Load errors disable its controls until retry succeeds. Explicitly unchecking all lenses saves an empty combination rather than inheriting defaults.
- Inclusive language additionally admits 43 reviewed native pattern IDs as advisory-only occupational, expression, and accessibility checks. Existing safe alternatives remain individually applicable; personal pronouns, neutral identity descriptions, and diagnoses are not inferred or rewritten.
- **Formulaic writing** is an opt-in English lens using Slopless 0.2.38 with 29 explicitly selected rules; the [complete inventory](WRITING_SLOPLESS.md) records all 48 exclusions. It reviews stock rhetoric, clichés, wordiness, complex-word density, redundancy, repeated openings/transitions/words, layered qualifications, exclamation density, em dashes with no whitespace on either side, emphatic punctuation from proselint, and `retext-quotes` quotation marks and apostrophes that differ from the note’s prevailing straight or curly style. Consistent authored punctuation is preserved; quote and apostrophe conventions are inferred independently and recomputed from the current note after incremental edits. A paragraph whose only exclamation marks form one run (“!!”) receives emphatic-punctuation advice without a second exclamation-density finding. Slopless `smart-quotes` is excluded because it duplicated the same marks; Ignore decisions saved for a curly apostrophe under it still apply to the apostrophe-style finding. Findings are advisory with contextual examples and persistent reversible Ignore, with no Slopless-generated Apply/Fix all or AI-authorship score. Shared concerns retain their contributing lenses; only an enabled lens may supply a reviewed Apply action. Different concerns on the same span stay separate; identical occurrences share a card. Slopless rules use protected prose; quote style uses the typography projection and its mark-only fix. All rule code initializes eagerly and executes in the background prose worker after debounce; stale results never reappear after typing.
- Long-note analysis memoizes unchanged Slopless tokenization through a bounded per-analysis cache (4,096 entries, 2 MiB estimated weight), returning independent token copies and clearing on completion/failure. The pinned punctuation rule receives paragraph-local coordinates and reports against the original paragraph, avoiding document-prefix padding while retaining its exact relative findings. All 29 Slopless rules and other package coverage remain available. Exact paragraph-local retext/textlint results are reused across edits in worker caches capped at 8,192 entries and 4 MiB each. The worker retains one note’s unchanged Markdown block maps, reparses safe paragraph/heading edits locally, and rebases UTF-16 positions. Structural changes parse the complete note; reference-context changes invalidate retained maps. Document-wide policy is recomputed from the complete current projection. Full-scan equivalence tests cover edits, paragraph moves/splits, protected Markdown and changing punctuation conventions.
- **Proofreading — grammar and punctuation** uses `@textlint-rule/textlint-rule-no-unmatched-pair` 2.0.4 to flag unmatched opening marks with advisory examples, without guessing where to insert a closer. This is not a complete nesting, lone-closing-mark, or curly-English-quote validator. **Proofreading — consistency** uses `textlint-rule-terminology` 5.2.16 with 29 reviewed technical names defined in `core/writingTextlintModel.js`, including JavaScript, GitHub, MongoDB, PowerShell, and WordPress; its default terms and external file configuration are disabled. Exact safe fixes preserve each name's canonical capitalization. **Clarity — plain language** uses the pinned `Microsoft.Acronyms` Vale rule for unexplained three-to-five-letter uppercase acronyms, retaining its familiar exceptions plus ATM/PIN. Definitions anywhere in eligible prose count, including before or after first use; matching lowercase expansions such as “service level objective (SLO)” also work across soft wraps within one projection region. Words that spell out an acronym in a forward or reverse definition receive no wordiness advice. Definitions cannot bridge protected content. Ordinary lowercase headwords from the bundled English dictionary also suppress capital-word false alarms such as KEEP/SHORT. Hyphenated expansions contribute separate initials, including conventional eX initials such as user-experience design (UXD). Reverse definitions such as “SLO (service level objective)” and plural forms such as “Digital Object Identifiers (DOIs)” also count. A limited noun-of-noun reorder recognizes “automatic insertion of semicolons (ASI)”. Recognition is syntactic and does not validate an expansion’s meaning. Acronym advice includes a labelled general example and Ignore, never a guessed expansion or automatic replacement. No number/unit spacing rule is bundled; `8.1Mib`, `10MB`, and `20ms` remain untouched.
- Vale is compiled from adapted, pinned source into the Go backend. Its 42 bundled YAML rules, pinned grammar dictionary, 162 pure Go grammar checks, and local sentence model initialize eagerly in memory; no executable extraction, helper process, host configuration, or vault text file is needed. Initialization never gates typing or shutdown.
- Eager local workers run the pinned retext/textlint checks and existing conservative nspell logic; the embedded Go Vale engine runs the selected write-good/proselint/Microsoft/Harper rules and Figaro grammar checks. The Plain language, Directness, Repetition, Consistency, Readability, Grammar & punctuation, and Formulaic writing check families independently request Vale through their UI groups, including when enabling a lens after cached results that lacked native evidence. The textlint kernel and plain-text parser initialize before worker readiness; an initialization failure is retryable. No engine download or remote analysis occurs at runtime. The application controls Vale configuration and does not load vault/host styles. Dependency pins and the reviewed phrase set are recorded in [WRITING_ENGINE.md](WRITING_ENGINE.md).
- Shared wording policy suppresses isolated synonym-list matches for reviewed familiar vocabulary while preserving useful phrase shortening and complex-word advice. Meaningful manner/frequency/degree and passive contexts are bounded pure decisions. Passive guards cover reviewed technical subjects, elliptical API descriptions, timed maintenance, physical/location states and possessive-gerund reactions; explicit actors remain reviewable, including after a time or location. Meaningful “there is/are” quantity/location, availability and no-reason/no-need statements stay unmarked; weak introductions such as “there is a need to” remain reviewable. Hidden prose, sentence breaks and paragraph breaks cannot supply the missing context. Independent grammar findings retain their corrections at overlapping positions.
- The mapping/configuration signature is version 25 and includes package pins, editorial policy version 9, spelling vocabulary version 5, reviewed terminology/acronym exceptions, and readability thresholds. The signature also pins curated Harper 0.1.0 and Figaro grammar policy 8. Fifteen reviewed Vale-port rules and 162 conservative Go checks are enabled under Proofreading; original Harper Rust/Wasm is not bundled. The [grammar contract](WRITING_HARPER.md) defines the selected inventory, adapted participle, suppose, and modal rules, morphology, exact-span actions, per-rule Ignore identity, duplicate-article suppression, and whole-block withholding around masked contextual text. The first 28 added Figaro checks cover reviewed verb constructions, number/mass nouns, its/their/your contractions, then/than and to/too, contextual word choice, verb complements, and possessive/plural forms. Replacements preserve capitalization and authored apostrophe style. Cross-line or noncontiguous edits remain advisory, and conflicting modals never receive a guessed replacement. Five further checks cover compound subject pronouns, initial have/am questions, and advisory adjacent-pronoun conflicts. Broader noun/verb and homophone contexts preserve object pronouns, possessive gerunds, proper-name modifiers, psychological affect, weighting samples, and dialect variants. Reviewed contraction corrections suppress overlapping generic determiner advice at the same position. A further 60 checks cover reviewed idioms, prepositions and gerunds, double-degree adjectives, split words, ordinal suffixes, personal-pronoun capitalization, comma spacing, and number-day compounds. Conditional counterfactuals receive advice only. Literal phrase meanings, mathematical variables, numeric separators, and uncertain lexical contexts are withheld. Split-word corrections suppress competing agreement only at the same occurrence. A further 60 contextual usage checks cover selected prepositions, common expressions, word choice, word boundaries and possessive/verb constructions. A separate noun-subject rule checks determiner/common-noun phrases and follows their head across one bounded prepositional modifier; collective, nominalized-adjective, invariant, coordinated, relative and mandative readings remain withheld. Countability now covers fewer → less for reviewed mass heads, preserving countable compounds such as fewer traffic jams. Accepted variants such as suffice to say, none the less, and intransitive crave for remain unchanged. Full native Harper parity remains out of scope.
- The prose worker retains the complete projection and raw observations; only Vale’s prose input and resolved review data cross into the UI. Resolution, suppression matching, display continuity, and card grouping run in that worker. A separate eager decision worker tracks saved and pending anchors and computes inactive IDs; the view never scans source to determine activity. Typing handlers only capture immutable document references and numeric changed ranges, with coalesced task scheduling rather than input-task or microtask scans. Worker failures never fall back to UI-thread analysis. Resolution explicitly states whether completed prose evidence is required; a replacement worker rebuilds a missing source cache before claiming completion. A failed rebuild keeps independent spelling results and reports partial failure with Retry analysis, excluding unmappable Vale results. Later engine success cannot silently hide that unresolved failure. Each run owns an immutable buffer ID, revision, exact source, effective configuration/version signature, and generation. Snapshot and decision bookkeeping waits for 100 ms of quiet; the analysis coordinator then applies its 500 ms debounce to one active/latest request. Explicit configuration changes start promptly; unchanged-input presentation changes reuse evidence. JavaScript worker initialization retains a five-second deadline; eager native rule initialization remains independent of editor readiness. Analysis receives a source-size budget: two seconds per started 64 Ki units, with a five-second minimum and thirty-second maximum (UTF-16 units in workers, UTF-8 bytes in Vale). Resolve/recovery jobs use their nested document source for the same budget. Cancellation returns to the caller immediately and discards obsolete results. Prose and spelling workers cancel cooperatively without reloading packages or dictionaries; the next job waits for cancellation acknowledgement, and timeout/error still terminates the worker. Native Vale reaches cooperative checkpoints on its one worker, with at most one queued replacement and bounded intermediate work. Saving and shutdown never wait for that worker. Successful current results survive another engine's failure and are labelled partial while remaining checks run or fail. Disabled, unsupported, analyzing, complete, failure/retry, and no-selected-lens states remain distinct.
- A mapped Markdown prose projection excludes frontmatter, code, diagrams, math, blockquotes, link destinations, URLs, explicit paths, recognized filenames/identifiers, and balanced straight/curly single or double quotations within a prose block. Escaped quotes and in-word apostrophes are not delimiters; unmatched quotes remain prose. Wiki targets, fragments, and complete embeds are protected by the shared target-first syntax parser, including hidden destinations behind aliases. Safe explicit aliases remain prose. Spelling applies the same wiki protection alongside its independent exclusions. Source ranges are half-open UTF-16 offsets; Vale's one-based inclusive code-point spans are converted explicitly. CRLF, entities, escapes, and markup keep source mappings; discontinuous or ambiguous phrase mappings never offer edits. A separate typography projection exposes quotation delimiters only to quote-style review; its special fix validates every changed delimiter and leaves enclosed source bytes untouched.
- Exact equivalent observations merge while retaining provenance. Native and local comma-spacing observations with the same minimal source edit share the local occurrence identity and one Apply action even when their surrounding text ranges differ; distinct replacements or intentions remain separate. A narrow passive participle may merge with its uniquely anchored auxiliary phrase; overlap alone never merges findings. Unknown rules or invalid ranges are rejected with retained reasons and partial status. Unreviewed simplifications stay visible as contextual advice without unsafe actions; persisted review suppressions remain in bounded run evidence. Conflicting editorial goals withhold actions; safe alternatives for one goal remain separate choices. There is no confidence score or whole-passage rewrite.
- Current eligible words/phrases receive dotted editor underlines using the existing spelling treatment. Hover opens the approved menu surface with an explanation, readable before/after wording for available fixes, verified **Apply** choices, and **Ignore this occurrence** saved for this document. Actions precede one before/after comparison. The first two replacement choices are visible; **More alternatives** reveals the rest and focuses the first newly exposed choice. Hidden choices do not participate in Tab navigation. Overlapping findings share the popup. Put the caret in a marked range and press **Ctrl/Cmd+.** to open and focus its first action; Tab/Shift+Tab traverse actions, leaving at either end returns to the editor, and Escape closes it. The shortcut is consumed even while no current suggestion is ready, avoiding a native emoji-picker fallback. Writing popups remain inside the visible editor/viewport intersection, clear of the sidebar. Hover does not move the caret. Source edits close popups synchronously and invalidate edited-paragraph and document-dependent marks. Unchanged-paragraph underlines map through edits and remain display-only, with disabled popup actions until fresh checks arrive. Structural Markdown changes (including fences and reference definitions) invalidate all underlines. All actions also revalidate their displayed snapshot. Results remain available inline while the pane is closed or suppressed in Pure mode. Decorations mark existing text and introduce no replacement widgets or saved syntax. Existing ordinary-link, explicit wiki-alias, and reference-link widgets receive label-only dotted marks and the same hover actions; hints crossing a label retain their original finding identity. The popup retains link destination/title information. Normal link activation and source/cursor transitions stay intact; destinations and reference definitions remain protected.
- **Ignore this occurrence** and **Accept “ACRONYM” in this document** persist in version 1 `.config/writing-decisions.json`, keyed by note path and analysis language. Acceptance suppresses all current/future undefined-acronym occurrences in that document, while other advice stays active. Occurrence records contain the canonical kind, exact source text, and up to 64 UTF-16 units of surrounding context on each side (surrogate boundaries stay intact). The initial immutable source snapshot reaches the note’s review controller before the first edit is tracked, even when the editor mount event precedes controller selection. Known source edits outside the target refresh its context and queue a debounced reanchor save. Reload requires one full-context match or exactly one candidate supported by an unchanged side of at least 12 trimmed characters and two words that itself appears only once in the source. Actual CodeMirror changed ranges govern remapping; touching/deleting the target stores `exactOnly: true`, which disables fallback but allows original-context Undo to reactivate it. Ambiguous full contexts never fall back to a guessed offset; changed targets and unidentifiable occurrences stay visible. Ambiguous identical contexts are refused when creating a decision. A stale action cannot create a decision for a different source or document. Renames and moves performed inside Figaro transfer both document lens preferences and review decisions to the new path, including descendant notes and merge collision names. Stable decision IDs, language, anchors, exact-only status and unknown fields survive, with Restore still available after restart. A storage barrier drains active writes before the native mutation and routes queued writes through the retained controller's updated path. The rooted adapter validates both records before moving, serializes with ordinary writing-state writes, and restores prior metadata on write/path/link-rewrite failure. Existing destination writing entries are never overwritten; conflicting or malformed records stop the operation. External filesystem renames are not inferred.
- **Document-gap policy:** Grammar policy 8 adds independently authored CountableAmount and advisory-only CommaSplice checks. CountableAmount spans the complete visible “amount of times/requests/errors” phrase, including “an” when it needs to become “a”; measured amounts and noun compounds remain unchanged. CommaSplice establishes bounded finite-clause context, permits an opaque technical subject, and preserves conditional/relative clauses, short rhetorical lists, parenthetical remarks and serial coordination. These two checks explicitly use local source eligibility; all other grammar rules retain whole-block protection. No punctuation rewrite is guessed. Perfect “had saw” becomes “had seen” only with an explicit object and bounded reviewed adverbs; tool nouns remain intact. Technical spelling recognizes args, backoff, Ctrl, debounce, fn, lifecycle, SIGINT, unref and WHATWG, including regular reviewed compounds. PascalCase and dotted members with an internal-capital identifier segment stay opaque, while ordinary “Hello.World” and slash/dot spelling errors stay eligible. Literal temporal “just,” bird-food phrases, exposed weather conditions, tired states and observed past scenes do not receive mismatched tone/cliché/actor/generalization advice. Other meaningful review remains selected by its existing lens.
- **Saved review decisions** appears in both the pane and Pure picker. Its disclosure lists 25 records at a time, with **Show more saved decisions**, and per-record **Restore suggestion** or **Review acronym again** buttons. Occurrence rows show context and mark unidentifiable targets **Inactive**. Records remain removable even when their source text no longer matches or their lens is disabled. New decisions take effect only after successful persistence; known source edits asynchronously update existing and pending anchors and queue metadata-only saves. New Ignore actions reconcile edits made during storage and persist their safe anchor before finishing. Pending storage commands remain immutable for idempotent retry. Background tracking errors keep review unavailable and offer Retry review decisions; source edits remain queued for safe recovery. Loading/saving and errors are visible. **Retry review decisions** retries a load or the same uncertain pending mutation; **Reload saved decisions** reconciles an uncertain result and releases it. Repeated reload requests share one operation. Reload waits for the active tracking job before installing the loaded decision set, matches tracked records by ID, and drains subsequent edits against that set without rewinding the source. Failed tracking leaves its edit queued for recovery; removed records cannot reappear from an older response. A concurrent load failure always exits Loading into a retryable load state, independently of tracking errors. Retry reloads the authoritative decisions before draining queued edits or retrying an uncertain command; a failed load never resumes tracking against the old set. Definite record/file-capacity rejections explain how to make room, release the rejected command, and permit Restore without restarting. Decisions are not copied by **Apply to all documents**. Per-document use cases isolate delayed responses and reject overlapping mutations; rooted read/modify/write preserves other documents and unknown fields, refuses corrupt/newer data or colliding IDs, and atomically writes private permissions without changing notes or Git refs. Identical commands are idempotent. Limit 1,000 decisions per note and 16 MiB per file; oversized records are refused.
- Spelling findings also offer **Add to dictionary**. Version 1 `.config/spelling-dictionary.json` stores `{"version":1,"words":[]}`. Matching is case-insensitive, normalizes curly apostrophes and canonically equivalent Unicode accents to NFC, and applies across notes in this vault and across restarts. English US/UK also accept each saved base word’s regular noun plural (`-s`, `-es`, consonant-`y` to `-ies`) and singular/plural possessives, including terminal straight/curly apostrophes after an accepted s-ending base or plural. This also applies to existing dictionaries without rewriting them. Adding a singular possessive also accepts its base and regular plural family. Adding an s-ending terminal possessive also accepts the unpossessed spelling without guessing a singular. Other internal-apostrophe entries remain exact; irregular plurals, verb endings, and prefixes are not inferred. Spanish keeps exact-word matching. Lookup normalization preserves original note text and UTF-16 offsets. The pure plan accepts s-ending terminal apostrophes and validates one NFC-normalized word (at most 256 UTF-8 bytes) and at most 10,000 accepted words, deduplicates additions, and preserves unknown fields. The rooted adapter serializes read/modify/write under the settings lock and atomically saves with private permissions. Missing dictionaries read as empty without a write; malformed/newer files and outside symlinks are preserved and rejected. A save failure leaves existing effective words and the marked suggestion intact, displays a retryable error, and never rewrites note text. Proofreading owns all spelling marks and right-click spelling actions. Both honor accepted words; deselecting the lens stops spelling review.
- Contraction checks distinguish “their/your/its going to be” from possessive gerund subjects with a later finite verb. “Their going to be late” can become “They're going to be late”; “Their going to be late worried us” stays unchanged. The bounded context includes infinitive passive/perfect auxiliaries and negation/modifiers; it is not a full clause parser.
- Findings group by sentence within a prose block, falling back to the block. Groups follow source order; findings are advisory and ordered by position, kind, and deterministic occurrence key. Counts include unique eligible findings, independent of evidence count. Each suggestion uses the approved rounded, borderless `.ui-suggestion` background, with shared standard buttons for its actions. The presentation combines identical kind/intent/text/message/replacement sets into one card, retaining every occurrence. Previous/Next cycles the card’s selected source occurrence. The pane mounts four distinct cards initially and adds four with **Show more suggestions**, independent of passage size; counts distinguish suggestions from occurrences. **Details** explains why the rule fired, its limits, and reasons to retain wording. Raw evidence is built only on opening **Technical diagnostics**. Automatic results never move the editor selection.
- Selecting a finding focuses its exact source range in CodeMirror. **Apply** synchronously checks owner, revision, configuration, source text, range, and mapping; mismatch changes nothing, announces a refresh, and reruns analysis. A verified contiguous phrase edit is one isolated normal undo transaction. Passive advice includes a labelled general before/after example but has no automatic edit. **Apply “…” to all N occurrences** is available for identical single-choice terminology, explicitly reviewed spelling corrections, reviewed complex-word/wordiness, punctuation-spacing, repeated-comma, or sentence-spacing suggestions after all running checks finish. It validates every range against the same source/snapshot, rejects stale or overlapping plans without any edit, and dispatches one isolated Undo/Redo transaction. Contextual advice, article/quote edits, ambiguous alternatives, and mixed-form consistency remain individual. **Ignore this occurrence** changes no source and follows the durable language/path-scoped decision contract above. Display identity follows unchanged occurrences independently from edit authority. Row removal moves focus to an available finding or the results container.
- Pure suppresses the existing pane intact. Its transient title-bar lens button or the same shortcut opens a temporary picker using the same controls and saved choices. Escape closes the picker, restoring its trigger; outside pointer input closes without stealing focus. Leaving Pure restores the mounted pane. Compact normal windows reuse the right-pane overlay so the editor retains usable width.

The pane omits passage excerpts and the repeated hover/shortcut hint. Each suggestion title includes an accessible source-link icon; activating it selects that occurrence in the editor. Apply, Ignore, and Details use the approved standard button surface. Repeating the selected Calendar Timeline control returns to Month; repeating Gantt returns to Board.

### 7.7 Document Outline
- The compact top-right Outline launcher is hidden for non-Markdown files, and remains visible and highlighted while Document outline owns the right pane. A Markdown note without headings keeps it visible but disabled with an explanatory tooltip and accessible description. While sticky headings are visible, its grid row follows the complete hierarchy so the control never overlaps a heading.
- Every mounted Markdown buffer also shows compact **Raw Markdown**, **PDF**, and **Writing lenses** icon launchers, in that order directly beneath Outline. They reuse the approved icon button, remain available when a note has no headings, toggle their shared right-pane mode, expose `aria-expanded`, and disappear with the complete launcher rail in Pure mode or when the shared editor does not own the active tab.
- Clicking an Outline, Writing lenses, Raw, PDF, or History launcher while the editor is focused preserves typing focus and the cursor when opening, switching, or closing the pane. Keyboard/assistive activation retains normal entry behavior, including focusing the current/first Outline heading or Writing lenses controls. Explicit clicks inside the sidebar take focus normally; deferred pane updates must not refocus the editor over those clicks.
...
- Heading navigation observes actual changes to sticky-strip height so late layout cannot cover the target. It does no frame polling, makes at most six height-triggered corrections, and releases the observer on the next user input or source/tab change. Pending corrections also reject a changed cursor or editor owner.
- Clicking or pressing Enter on an outline or sticky-hierarchy heading moves focus and the normal CodeMirror cursor to that source heading and aligns it at the top of the available editor viewport, below any sticky headings. Navigation respects the document's scroll limits and leaves source unchanged. A later tab switch, History opening, Raw Text Preview opening, PDF Preview opening, Writing lenses opening, or × close releases the pane without changing note content.
...
- Search indexes the static Markdown, Macros, and Shortcuts reference plus curated Settings destinations. Matching is local and does not execute application commands. Arrow Up/Down changes the active result and Enter activates it. Pointer presses retain DOM focus on the search field until activation, and removing the chosen result must not let the resulting transient focus loss dismiss the popup. A Help result keeps Help open, selects its topic, scrolls to and briefly highlights the exact row; a Settings result deliberately closes Help, opens or de-duplicates the Settings tab, scrolls to the exact section, and moves a quiet focus/highlight to the target control. Results show their Help or Settings breadcrumb so syntax and configuration remain distinguishable.
...
- While open, the application surface is inert, Tab and Shift+Tab remain inside the modal, and a visible **ESC to close** cue identifies the keyboard dismissal. Escape follows the dialog's cancel path regardless of which field or embedded editor owns focus, and closing restores focus to the invoking control. An open nested picker consumes the first Escape. Transactional Table, Chart, and Mermaid editors instead expose the shared inline **Keep editing / Discard** confirmation when their temporary draft differs from its opening state. The Table Editor's Keep editing action and Escape-from-confirmation restore the initiating cell/control and text selection, not the Cancel button. Opening another modal cleanly cancels the previous one.
- Only the Table, Mermaid, and Chart editor compositions add the approved lower-right resize handle. Primary-pointer dragging changes width and height together; a focused handle uses Arrow keys in 24 px steps, Shift+Arrow in 8 px steps, and Home to restore the CSS-managed default. The pure geometry policy enforces a 480 × 360 px minimum while space permits, yields to smaller windows, and keeps 24 px viewport margins. Sizes are temporary for that modal session. Escape during an active pointer gesture restores its starting geometry without closing the dialog; the next Escape follows the ordinary nested-picker and dirty-draft rules.
...
- The machine-local session record (legacy fallback `vault/.config/session.json`) carries the workspace session (tabs, active tab/document, current per-file cursor selections, expanded folders, pinned tabs, and last focused tree row). Cursor updates stay in memory until a tab switch, window blur or quit, and are installed before the restored active file is mounted.
...
4. Initialize CodeMirror and the tab manager, install native close/unload protection and restore Auto-Save, then initialize the breadcrumb coordinator and file-tree handlers; recreate inactive tabs as metadata only, then read and mount the one selected file with its saved cursor state. Await the scheduled document replacement before considering that restored activation complete. A very large Markdown source is installed in two line-boundary chunks without entering Undo history. Its leading frontmatter and only the image, diagram, table, or math stages indicated by the source are prioritized across guarded animation frames; dormant authoring fields attach afterward without rescanning the outgoing document. Switching files cancels unfinished stages. Resolve operating-system launch documents through the import/keep-outside choice before starting full-vault work.
5. Eagerly initialize Calendar, Kanban, Graph, global search, backlinks, History, Activity, Document outline, PDF preview, Raw Text Preview, and writing lenses. Begin worker, Vale, dictionary, and writing-preference restoration without awaiting them before editor reveal. After the selected document mount settles, keep the editor concealed through two animation frames so CodeMirror can settle its document-dependent measurements, then reveal the authoritative buffer and call `StartVaultLoad()`.
...
7. Show the un-tabbed workspace overview only when no session or launch document restored a tab. Hide successful index progress once the index and tree settle; the status bar can still indicate writing preparation. Publish `window._appReady` after the remaining eager startup promises settle. Feature failures leave the editor available.
8. Editor saves atomically write to disk and acknowledge that version before optional Git history, followed by task-metadata and index updates. Subsequent disk writes and save-and-exit do not wait for Git. Follow-up failures cannot roll back or reclassify a successful disk save; genuine write failures and external conflicts retain the draft and normal recovery choices. The Auto-Commit toggle remains event-driven and records only a successfully saved file. Single-file history captures the target through the vault root, then writes Git objects outside the content lock without walking unrelated working files. A newer edit can save during those Git writes and remains eligible for its own revision. Rename, move, merge, delete, and recovery coordinate through a separate path lock before the content lock. Unrelated staged changes, including deletions, are refused; Git failures preserve the saved note and restore staging after a failed commit. A changed Git index or HEAD detected after snapshot storage aborts history rather than overwriting that observed staging state.
...
13. Session persistence uses a machine-local record per vault, falling back to a legacy `vault/.config/session.json`.
...
- Custom columns are derived from hashtags in use; an emptied column stays on the open board until it is left, and its color is kept for later reuse. `todo`, `wip`, and `done` remain available.
...
- Each changed dirty buffer is parsed once for its cards and columns. Unchanged task projections retain the board and card DOM through prose edits; source-line shifts, task text, tags, and completion changes still refresh it. Saved or closed buffers leave the projection cache.
...
- Session persistence: tabs, cursor positions, expanded dirs saved to a machine-local record per vault (legacy `vault/.config/session.json` is read once).
...
- **Pure mode**: one compact Appearance section owns enabled-by-default **Typewriter scrolling**, **Focus scope** (`Off`, `Phrase`, or `Paragraph`, default `Off`), and disabled-by-default **Adapt text to window size**. Focus scope and other select-only listboxes use a body-level overlay whose pure placement plan keeps the popup aligned with its trigger, flips it above when necessary, and clamps it inside the viewport rather than inheriting transformed or clipped panels. Inline Help and sidebar-search results remain deliberately contained in their own surfaces. A profile without a stored Typewriter value uses Typewriter scrolling; focus and adaptive behavior remain explicit opt-ins. With an active file and collapsed sidebar Figaro always applies the edge-to-edge shell and writing contract from §1.1 regardless of an open details pane. These behavior preferences and the expanded/collapsed sidebar state persist independently, while the vault session remembers the active buffer; when they restore, Figaro reveals that buffer directly in Pure mode without an intermediate expanded-shell frame.
...
- **Default Text Size**: −/+ buttons adjusting the permanent editor baseline from 70% to 150% in 10% steps. The displayed 100% baseline is 16.2px, the buttons and value share one continuous themed background, and changing it clears temporary scale overrides from currently open buffers. Ctrl/Cmd+mouse-wheel over the main editor changes only its active open buffer, updates its visible status-bar Scale value, and the status-bar **Scale** button returns it to this Settings value.
...
- Resize grip at the physical window bottom-right, independent of the buffer-status width: drag to resize, calls `WindowSetSize` via the Go backend. It stays visible outside Pure mode and is hidden in Pure mode.
...
- Window state and installed browser paths are host-specific and must never be stored in `vault/.config/settings.json` or the session record.
...
- Vault configuration is eligible for Git tracking. Startup removes the exact legacy `.config/` ignore line previously inserted by Figaro, preserving other rules and line endings. It does not create an otherwise absent `.gitignore`, stage files, or create commits. Preference writes do not broaden the per-note automatic history contract.
...
- Toggleable panel on the right side of the workspace (resizable via pointer or the same physical-direction separator keys, a shared 220px minimum with the maximum set by the remaining workspace and editor floor).
...
- Paste priority is internal Figaro Markdown, explicit plain-text bypass, a validated high-confidence table, image, URL-over-selection, protected source context, semantic rich HTML, then native/plain fallback. A spreadsheet table therefore wins when the clipboard also advertises the selected cells as an image; an ordinary image still wins over general rich HTML. Native paste uses CodeMirror's Markdown extension; the pure URL plan and editor syntax adapter provide the identical one-transaction result for Vim `p`/`P` and Async Clipboard menu paste. Block conversion adds safe blank-line boundaries; inline paste into revealed table source never injects a block.
...
- Ctrl/Cmd+mouse-wheel uses the same range and steps but stores the result only on the active open file buffer. Every handled gesture updates the Scale value in the persistently visible normal-mode status row. Tab switches restore each buffer's value; closing it discards the override; neither portable sessions nor recovery snapshots serialize it.
...
- **Avoid outgoing presentation work**: Markdown presentation and history are removed together before the source replacement. The target's pure stage plan prioritizes only syntax present in its source; helpers for syntax typed later attach after the ready boundary.
...
- Code block replaced with a `DiagramWidget` — renders SVG via `mermaid.render()` or `vegaEmbed()`. Mermaid reuses a bounded source cache. Vega/Vega-Lite retain at most 64 outputs and 4,194,304 UTF-16 code units of keys/SVG, keyed by effective appearance/data, container geometry and font generation; external URLs and expressions reading time, randomness or window/screen state bypass reuse. Concurrent identical requests coalesce; failures remain retryable. Each mount has unique SVG IDs and preserved local references. Prepared live SVG subtrees survive source reveal and viewport removal in a per-editor cache capped at 32 previews and 4 MiB estimated SVG/key/node data. Their renderer identity, source, appearance, fonts and responsive width must still match; volatile/external-data charts bypass retention. Returning to a valid prepared preview reattaches it before paint without SVG reparsing or the generation delay; controls and observers belong to the new wrapper. Held-key bursts defer prepared Mermaid attachment until quiet; Vega restores immediately. Active composition defers both, protecting its input region from SVG layout work. Source-height measurements separately retain up to 256 exact source/width/typography entries under a 1 MiB estimated-data cap, clearing on explicit typography or completed fonts. The live queue for new or invalidated output waits 120 ms after typing, composition or scrolling, then an idle opportunity; further input cancels the pending slot. Running DOM render work cannot be preempted. Source edits, appearance/font changes, resized containers and disposed widgets invalidate outdated results. Unstyled Mermaid live/editor previews retain the temporary application palette and editor-surface canvas; explicit Mermaid styling remains authoritative, and printable HTML/PDF use the unchanged authored source.
...
- The Chart Editor gives its borderless live preview the larger side of the modal and
  ...
  control and asks before discarding an unapplied chart configuration. The generated chart uses the full writing
