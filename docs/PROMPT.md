...
figaro is a desktop Markdown workspace with vault-based file management, quick-note capture, a hashtag-driven Kanban board, a date-aware calendar, backlinks, sticky heading hierarchy and a contextual document Outline, session-persistent tabs, local Git history, seventeen themes, sixteen bundled fonts, optional Vim mode, KaTeX math, live diagrams, editable Draw.io SVGs, and interactive browser-backed PDF export. All content lives in a folder chosen by the user: no accounts, cloud service, sync engine, or proprietary note database.
...
- **Top bar**: sidebar toggle and app title/Home control on the left; a clear draggable center region; a `?` Markdown-cheatsheet button immediately before Settings, followed by native-style minimize/maximize/close controls on the right. The help button and other action buttons deliberately retain the link-style pointer cursor.
- Calendar and Kanban live in a fixed footer below the file tree. Calendar expands inside the left sidebar; Kanban and Settings open or return to their single corresponding workspace tabs. Clicking the already active persistent destination plays a short reduced-motion-aware exit transition before closing that destination tab. Clicking the Figaro name opens the un-tabbed workspace overview.
- **Status bar** fixed at the bottom: left shows live status text ("Ready") and an optional adjacent action such as deletion **Undo**; right shows cursor position, the active file buffer's **Scale** value, reading time estimate (words ÷ 200 wpm, minimum 1 min), word/line/char count, file encoding, markdown file type, backlink count, and committed **Changes** count. **Scale** is a native, link-styled button that resets the temporary buffer scale to the permanent Settings default. Backlink and Changes counts are native buttons: unavailable zero states are disabled, while actionable counts preserve their compact link-like styling and pointer cursor and open with Enter/Space. A plain-language **Save to history** action appears only while the active file has a version waiting to be recorded; it safely saves pending text and records only that file, then hides again. The bar keeps its fixed 24px single-row contract at narrow widths, removing extended metadata and then lower-priority reading/word details instead of wrapping.

### 1.2 Left Sidebar
- The top of the sidebar is the **global note search** combobox; its keyboard-navigable result list opens below the field, exposes the selected option through `aria-activedescendant`, and keeps focus in the field when Escape clears the popup. Above 120 matches it mounts a moving 96-row window while retaining the complete logical list, scrollbar range, accessible position/set size, and keyboard reachability.
...
- The remaining space is the **Vault** file tree. It is one page-level Tab stop whose roving focus belongs to the last focused row (falling back to the active document or first visible row), rather than to a generic container. Once inside, Up/Down traverse visible rows without opening them, Home/End jump to the boundaries, Right expands a folder or enters its first child, Left collapses a folder or returns to its parent, and Enter/Space activates the focused row. The active document alone owns the strong selected surface and `aria-current`; it follows tab switches immediately even when keyboard focus remains on another tree row. Markdown and files recognised by CodeMirror's language registry are clickable; unsupported/binary assets remain keyboard-navigable, expose their disabled state, and are greyed out at 55% opacity. Above 400 visible logical rows the tree mounts a moving 160-row window; hierarchy levels, focus, current-document selection, context-menu focus restoration, drag behavior, scrollbar range, and distant keyboard activation remain path-based rather than DOM-position-based.
- A fixed tool footer keeps Calendar and Kanban reachable beneath the tree. Calendar expands a monthly panel between the tree and footer without taking ownership of the right pane. Kanban uses one de-duplicated workspace tab. The open Calendar panel keeps its intended height when the vault tree is long; the tree and overflowing Calendar results scroll independently.
- The sidebar can be resized from **225px to 500px**. Collapsing it leaves a **44px tool rail** rather than removing navigation; Quick note remains directly usable there, while selecting Calendar expands the sidebar and opens the panel. Expanded folders, the active document and last focused tree row, recent files, and search filters are stored locally for the current webview profile.

### 1.3 Right Sidebar
- The right sidebar is closed by default and is reserved for **History**, **Document Outline**, **Raw Text Preview**, and **PDF Preview**. Opening one replaces the others; the left-sidebar Calendar can remain open independently. While closed, the mounted zero-width pane is `aria-hidden` and inert, so its Close control and descendants cannot remain in sequential keyboard focus.
- Clicking or pressing Enter/Space on an actionable committed-history button opens the History view for the active file.
- A compact hamburger launcher near the editor's top-right appears only for Markdown notes with headings. It sits below every visible sticky-heading row rather than covering that hierarchy. It opens **Document outline**, which lists typed nested H1–H6 headings, ignores frontmatter and fenced-code examples, highlights the current cursor/reading section, and focuses the editor at the selected heading. The launcher hides while this pane is open and returns when it closes.
- Its width can be resized from **240px to 480px** for the current session.

### 1.4 Workspace
- On the first shell frame, the workspace reuses a webview-local mirror of the last confirmed bundled theme and fonts while vault settings load as the authority. It then restores saved tabs as metadata and reads and mounts only the selected file before calling `StartVaultLoad()`. The active editor remains usable while the initial file tree, full-vault search/planning index, preferences, and language parsers warm concurrently. The frontend subscribes before the start request and immediately reconciles `GetVaultLoadStatus()`, so every phase is observable without a subscribe/start race. A compact accessible indicator in the bottom-left status bar progresses from **Discovering notes…** through the exact `loaded / total` Markdown-note count and finalization; it hides on success and remains visible on an index error. `window._appReady` is set only after the eager work has settled.
- A horizontal **tab bar** across the top. Compact, contiguous tabs use a responsive 104–200px width, preserve both ends of long filenames, show a muted parent path for nested notes, keep their close controls accessible, and mark the active document with an accent along its top edge. The full filename and vault path form each tab's accessible name and tooltip. Left/Right/Home/End switch tabs while retaining focus on the newly mounted active tab, so multiple arrow presses continue through the rail. Vertical mouse-wheel input over the rail cycles in either direction and wraps at the ends; small high-resolution deltas accumulate before one switch, while horizontal or modifier-assisted wheel gestures retain native scrolling/zoom behavior. The visual scrollbar is hidden, the active tab is always scrolled fully into view, and subtle themed edge fades indicate additional tabs. The keyboard-accessible all-tabs menu appears only while the rail genuinely overflows and lists each complete title plus parent path. A dirty tab uses a compact warning dot. In the file tree, the active document uses the sole selected surface; clean background tabs are deliberately unmarked, while every unsaved file buffer gets a warning dot and an assistive “Unsaved changes” status. Tab switches and dirty transitions patch those mounted states without rebuilding the tree. An optional, disabled-by-default breadcrumb can show the active note's vault-relative path between the tab rail and editor.
...
| **Kanban** | `kanban` / `kanban-board` | The persistent destination board and a hashtag-focused board respectively; each ID is deduplicated while open |
...
- The tree uses `tree`/`treeitem`/`group` semantics, row levels, expanded state, multi-selection state, and roving focus. Tab enters only the current focus row; Up/Down, Home/End, Left/Right, and Enter/Space follow the standard tree behavior described above. Focus navigation does not pretend to open or select a document: the active editor/Draw.io tab alone owns the primary selected surface and `aria-current`, while Ctrl/Cmd multi-selection remains independent. Structural refreshes preserve the focused row and scroll position, and the existing accent outline appears only for keyboard focus.
...
- Clicking a file focuses its row, opens it in the editor (replacing the current file tab), and transfers the sole selected-document state only after activation succeeds. Clicking a folder focuses and expands/collapses it without creating a competing selected-document surface.
...
- **Ctrl/Cmd+Click** adds/removes a vault Markdown file to the multi-selection (highlighted with an accent-tinted background and outline). External root shortcuts do not join this selection.
- **Ctrl/Cmd+X** cuts the focused tree file or folder; the next **Ctrl/Cmd+V** moves it into the focused folder or beside the focused file through the same safe move/merge and link-rewrite workflow as drag/drop, then clears the cut clipboard. **Ctrl/Cmd+C** copies instead, and copied items remain available for repeated pastes during the current application session. The context menu presents **Cut, Copy, Paste** in that order and shows faded platform shortcut hints, plus **F2** for Rename and **Delete** for Delete. Raw Text/PDF preview commands remain in the editor context menu and Properties rather than the file-tree menu. A copy paste first saves dirty open files inside the source so the duplicate matches the visible editor content; a dirty Draw.io SVG must finish its independent explicit save first.
...
- Multiple vault Markdown files can be selected with Ctrl/Cmd+Click; external shortcuts remain single-target items.
...
| Cut | File or Directory | Marks the selected item for one deferred move; the next Paste uses the safe link-aware move/merge workflow. Shortcut hint: Ctrl/Cmd+X. |
| Copy | File or Directory | Places the selected item on Figaro's repeatable internal copy clipboard without changing it. Shortcut hint: Ctrl/Cmd+C. |
| Paste | File, Directory, or vault root (after Cut/Copy) | Moves a cut item or copies a copied item into a focused directory, beside a focused file, or into the vault root. Copy collisions receive a `copy` suffix and never overwrite existing content; moved/copied Markdown links preserve their resolved targets. Shortcut hint: Ctrl/Cmd+V. |
...
- Preview Raw Text and Preview PDF remain available from the editor context menu; PDF Preview also remains in Properties. They are deliberately absent from this file-operation menu.
...
- **Frontmatter / Properties**: a complete leading YAML frontmatter block is rendered as a compact, collapsed Properties card. Its left-edge disclosure arrow rotates 90° when expanded, remains in the same viewport position by reserving a stable editor scrollbar gutter, and collapses the panel from that same control; there is no separate close button. The structured panel offers PDF-layout controls for cover page, contents depth, physical page numbers, and a vault-relative print stylesheet; preview commands remain in the file-tree and editor context menus rather than appearing in Properties. A Spellcheck section selects the global fallback, either bundled language, or disables checking for that note. Picker listboxes may extend beyond the card, but their full visible surface remains above the following editor lines: hovering an exposed option keeps the picker focused and expanded, and clicking it updates the property without moving the document selection. Enabling a cover also exposes title, subtitle, author, and date fields. Other YAML remains visible as chips and **Add property** opens the source editor with completion; **Edit YAML** always exposes the original portable frontmatter. Notes without frontmatter get a subtle **Add properties** affordance above the editor; expanding it inserts the YAML skeleton with the first H1 as `title`, the OS username as `author`, today's local date, an empty-string `subtitle`, and the PDF defaults `cover-page: false`, `toc-depth: 0`, and `page-numbers: false`, then immediately shows the structured panel without moving the cursor into raw YAML. Home/document-start navigation, Vim `gg`, programmatic selection, mouse placement, and drag selection keep Properties rendered; only deliberate upward entry with Arrow Up or Vim `k` reveals raw source automatically, and moving out collapses it again. Custom PDF CSS is opt-in: **Create starter** proposes `pdf.css` beside the active note, copies the bundled comprehensive example only after confirmation, selects it, refreshes the tree, and opens it. Once a stylesheet is selected, **Upgrade copy** proposes a separate `-v2.css` path, copies the current starter, appends every original rule as a final override, selects the new path, and leaves both the source and any existing target untouched. Startup and export never create stylesheets. The panel makes targeted scalar edits only, preserving unrelated YAML and comments. Completion in YAML suggests `title`, `subtitle`, `author`, `date`, `aliases`, `tags`, `description`, `created`, `updated`, `status`, `spellcheck`, `cover-page`, `toc-depth`, `page-numbers`, and `print-stylesheet`; it also offers status, spellcheck-language, and vault-relative CSS-path values.
- **Raw text preview**: **Preview Raw Text** opens the exact Markdown source from the editor context menu in the right pane. It refreshes from the active/dirty note snapshot and saved source updates, preserves frontmatter, HTML, fences, whitespace, and delimiters, and uses `textContent` rather than rendering source as markup. Closing it or opening History, Document outline, or PDF Preview releases the same right pane.
...
- **Live preview**: Formatting markers (`#`, `**`, `*`, `~~`, backticks, link brackets/parens) are hidden on non-active lines while preserving layout width. Move the cursor to a line to reveal its raw markdown for editing. Bullet points render as styled bullets. Task checkboxes (`- [ ]` / `- [x]`) render as interactive HTML checkboxes that toggle on click. Links render as clickable widgets. Unaffected preview state is retained during ordinary cursor movement, and interactive decorations are limited to the visible editor region so large notes remain responsive. With the opt-in Vim rendered-block motion, Normal `j` and `k` deliberately place the cursor inside an adjacent block to reveal its source; Visual `j`/`k` always extends its selection into adjacent fenced source. Tables keep their interactive widget and enter the first or last cell. Live content observers receive the latest typing frame, while word statistics settle after a short typing pause; save and tab-switch snapshots stay immediate.
- **Stable rendered-block footprints**: in the editor only, Mermaid, Vega, and Vega-Lite fences, ordinary fenced code, multi-line display math, and GFM tables reserve the visual height of their Markdown source, including wrapped rows at the current editor width. Entering or leaving source therefore keeps following text fixed. Diagram SVGs and KaTeX display output scale down—but never up—to fit; a smaller result is centered inside a subtle dashed **Markdown footprint**. Code and interactive tables retain normal readable text and controls and scroll within the reserved height when their rendered content is taller. Loading and recoverable diagram errors stay inside the same slot. Inline math, images, frontmatter/Properties, links, tasks/checkboxes, and all other inline replacements deliberately retain their existing intrinsic behavior. Raw Text Preview, PDF Preview, and generated PDFs are unchanged.
...
- **Interactive tables**: rendered GFM tables expose a compact `delete` action directly beneath their left-side `table` fold helper. It stays visually quiet until hover or keyboard focus applies the theme's destructive treatment. When the measured left margin cannot contain its full label, the same action moves into a content-sized row above the grid instead of entering the sidebar or covering cells. It removes the complete table source in one normal history transaction, returns focus to the document editor, and is fully reversible with Undo. The table's measured root uses its header, separator, and body-row count as the immediate footprint, then the shared wrapped-source ruler refines it at the active editor width; a taller grid scrolls without scaling its nested editors or controls.
- Inline rendering of hashtags and markdown links with distinct styling.
- **Fenced code blocks**: triple-backtick blocks with optional language tag, rendered with monospace font, subtle background, syntax highlighting (via highlight.js classes themed with `--code-*-color` variables), copy button, and line numbers. Their opening fence, body, and closing fence determine a fixed editor footprint; excess preview content scrolls rather than shrinking text.
...
| `markdownTableCompartment` + `markdownTables()` | Interactive GFM table rendering, formatting, cell navigation, row/column editing, one-click undoable table deletion, fold-aware preview yielding, and rebuilt embedded cell editors when the active Vim extension changes |
...
- A long file tree cannot flex-shrink the open Calendar below its intended height. The tree yields the space and keeps its own scrollbar; Calendar results beyond the panel cap scroll inside the panel while the monthly grid remains visible at the normal window size.
...
- Collapsing the sidebar closes the expanded panel but leaves the Calendar icon in the 44px rail. Selecting it expands the sidebar and reopens Calendar. History, Document outline, Raw Text Preview, and PDF Preview remain independent on the right.
...
- `Inbox` is an ordinary vault folder: its notes participate in links, search, Kanban, Git history, file-tree styling, pinning, and external editing. Its built-in Mail icon and initial pinned position are overridable defaults; an explicit custom icon or unpin persists.
...
- The compact top-right launcher is hidden for non-Markdown files, Markdown notes without headings, and while Document outline owns the right pane. While sticky headings are visible, its grid row follows the complete hierarchy so the control never overlaps a heading.
...
- A direct `delete` button remains outside cell editing beneath the left-side
  `table` fold helper, with a constrained-width fallback above the grid, and
  removes the complete table as one undoable CodeMirror transaction.
...
- Vim `gg` leaves a leading Properties card rendered, while `k` at that top
  boundary deliberately reveals its raw frontmatter even when **Enter rendered
  blocks** is off.
- When enabled, Vim Normal `j`/`k` additionally stop at adjacent rendered
  fences and reveal their raw Markdown source. Tables stay interactive and
  receive the first/last cell instead. Operator-pending motions are unchanged.
- Visual `j`/`k` always preserves and extends the current selection through an
...
tab dirty and publishes `Save failed — <cause>` through the polite, atomic live
status text; the complete message remains available as its tooltip when narrow
layout truncation is necessary.
...
- Click or keyboard-activate the `?` button immediately left of Settings to open the Markdown cheatsheet. The closed popup is `hidden` and contributes no controls to the Tab order; opening moves focus to its Close button, and Escape closes it and restores focus to `?`.
- Reference table: headings, emphasis, strikethrough/highlight, Markdown links, conventional `[[wikilink.md|wikilink]]` syntax immediately afterward, images, lists, tasks, and the complete portable task due-date example `- [ ] task #todo [due 2026-08-14](2026-08-14.md)` immediately after the normal task row. Blockquotes and the complete quoted syntax for the six supported admonitions/callouts—Note, Warning, Info, Tip, Danger, and Example—are followed by code blocks, Mermaid, Vega, Vega-Lite, Draw.io SVGs, math, HRs, hashtags, footnotes, and tables.
...
- Browser `localStorage` keeps webview-local presentation state (selected date, recent files, search filters, the permanent default editor text size, text width, the disabled-by-default editor-breadcrumb preference, and a recovery copy of tabs). Temporary editor scale belongs only to the corresponding in-memory open file buffer: it survives tab switches, is excluded from both persistence layers, and disappears when that buffer closes. A missing or malformed session is ignored safely.
...
1. Paint the shell with the last confirmed local appearance mirror, begin bundled language-parser warming, restore webview-local UI state, and initialize backend-independent navigation and window controls.
2. Wait for Wails to publish the native `window.go.desktop.App` binding, then read and apply authoritative vault-backed theme, font, and tab-size settings and subscribe to vault notifications.
3. Load the portable vault session, initialize CodeMirror, the tab manager, breadcrumb coordinator, and file-tree handlers, and recreate inactive tabs as metadata only.
4. Read and mount the one selected file, including its saved cursor state. Resolve operating-system launch documents through the import/keep-outside choice before starting full-vault work.
5. Initialize Calendar, Kanban, global search, backlinks, History, Document outline, PDF preview, and Raw Text Preview, then reveal compact status progress and call `StartVaultLoad()`.
6. Start the initial file-tree read, Links and automation preferences, full Settings preference initialization, and any remaining language warm-up concurrently with the index. Native debounced vault notifications replace polling; content-only changes do not reload the tree, and Figaro's own saved snapshots do not request an already-known Kanban board again.
...
| Enter / Space | Focused file-tree row | Toggle a folder or open an editable file |
...
| Ctrl/Cmd+X | Focused file tree | Cut focused file/folder for the next internal Paste move |
| Ctrl/Cmd+C | Focused file tree | Copy focused file/folder to the internal clipboard |
| Ctrl/Cmd+V | Focused file tree | Move a cut item or duplicate a copied item into the focused folder, beside the focused file, or at vault root |
...
| Right-click editor | Editor | Context menu (Cut, Copy, Paste, Select All, Preview Raw Text, Preview PDF) |
| Tab / Shift-Tab | Editor source or list | Indent / dedent by the global Tab Size; interactive tables retain cell navigation |
...
- The sidebar contains a `.sidebar-content` wrapper that clips internal overflow; the file tree and an open Calendar provide their own vertical scrolling.
...
- The file tree scrolls independently enough to keep its lower items reachable without clipping. An open `.sidebar-calendar-panel` does not shrink against the tree's content height; it takes its capped share above the fixed tool footer, while the tree and overflowing Calendar results scroll independently.
...
- The 43px edge-to-edge container uses the same visual family as the title bar, with a subtle divider that separates navigation from the editor without creating a heavy second header.
...
### 20.2 Tab Styling
- Width: `clamp(104px, 18vw, 200px)`; tabs fill the 43px rail and meet at hairline separators without gaps.
- Inactive tabs are intentionally quiet. Hover and active states use theme variables, with the active tab identified by an accent along its top edge rather than a raised card or heavy pane border.
- Long titles use a middle ellipsis so both their opening and differentiating filename ending remain visible. Nested files also show a muted, end-preserving parent path; the icon and close target remain fixed-width, and every close control stays visible at a quiet opacity before hover or focus.
...
- Auto-save on switch, continuously updated cursor memory (including Settings detours and restart recovery), vertical-wheel tab cycling, middle-click close, right-click or Shift+F10 context menu, and selection-safe pointer-driven drag reorder that does not depend on native HTML drag events. Context-menu Up/Down navigation wraps, Home/End jump to the boundary, and Escape returns focus to the tab.
...
- **Default Text Size**: −/+ buttons adjusting the permanent editor baseline from 70% to 150% in 10% steps. The displayed 100% baseline is 16.2px, the buttons and value share one continuous themed background, and changing it clears temporary scale overrides from currently open buffers. Ctrl/Cmd+mouse-wheel over the main editor changes only its active open buffer; the status-bar **Scale** button returns it to this Settings value.
...
- **Enter rendered blocks**: Vim-only persistent toggle. It remains disabled until Vim mode is enabled; when active, Normal `j` and `k` enter adjacent rendered fences or the first/last interactive table cell instead of skipping the widget. Leading frontmatter has a narrower invariant independent of this setting: `gg` preserves Properties and `k` deliberately reveals its raw YAML. Visual selection through fenced blocks reveals their source regardless of this toggle so Visual mode cannot collapse at a preview boundary.
...
- The native Figaro pair keeps the same semantic roles and accessible text/link contrast in both light and dark modes. Their component treatment deliberately makes navigation, the contiguous active tab, selected tree entries, Settings cards, and the editor reading surface visually distinct; gradients remain subtle background atmosphere rather than a competing visual element.
...
- The active document uses the strongest selected surface. Clean background tabs have no file-tree marker; any unsaved file buffer uses a warning dot with assistive “Unsaved changes” text. Keyboard focus remains a separate outline, and Ctrl/Cmd multi-selection retains its accent-tinted background and outline.
...
- Ctrl/Cmd+mouse-wheel uses the same range and steps but stores the result only on the active open file buffer. Tab switches restore each buffer's value; closing it discards the override; neither portable sessions nor recovery snapshots serialize it.
- The active file's status-bar **Scale** button exposes the effective percentage and resets it to the current Settings default. Workspace, Settings, Calendar, Kanban, and other non-file views hide that action.
...
- Mermaid copies the root state into its temporary CodeMirror editor, while table widgets receive the same facets in their nested-cell profile and are rebuilt after a live setting change.
...
- The Mermaid Editor uses a temporary CodeMirror document. Side-by-side,
  ...
  a default cursor rather than the loading cursor. On wider windows the source
  and preview panes share the available width equally and grow together until
  the dialog reaches its 1180 × 780 px cap; below 820 px they stack vertically.
