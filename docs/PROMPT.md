...
### 1.4 Workspace
- Before the first vault tree is available, the workspace contains an early-painted, themed **Loading vault** notice rather than a blank surface. It begins with **Discovering notes…**, then presents the exact `loaded / total` Markdown-note count, a determinate accessible progress bar, and a finalization state. Native progress events and an immediate status snapshot cover both live updates and indexing that began before the frontend subscribed. The loading view is removed only after the restored document or Today dashboard has replaced it.
...
- A `.md` document passed by the operating system at application launch asks whether to import when it is outside the selected vault.
...
- **Smart URL paste**: pasting a URL over selected plain Markdown prose creates `[selected prose](URL)` rather than replacing the label. The same behavior applies while Vim is in Visual mode; existing link/code selections keep the normal paste path.
...
- `p` / `P` — paste OS-clipboard text after/before the cursor, preserving Vim
  ...
  back to that register if system clipboard access fails or is empty
...
3. Initialize CodeMirror, the tab manager, and the editor-breadcrumb coordinator. Load the file tree, vault-scoped appearance/pin preferences, and attach handlers. Native debounced vault notifications replace polling; content-only changes do not reload the tree, and Figaro's own saved snapshots do not request an already-known Kanban board again.
4. Restore persisted tabs after the tree is available, then resolve each operating-system launch document through the import/keep-outside choice. Show the un-tabbed workspace overview only when neither workflow opened a tab.
5. Initialize Calendar, Kanban, global search, backlinks, the History panel, and the sticky-heading/Document-outline navigator.
6. Load the saved theme, prose font, and code-font settings. A Settings tab initializes its own controls when opened.
7. Load the persisted line-number, Markdown-diagnostics, spellcheck, Markdown navigation, and Links style preferences, then load the Auto-Save interval and start its active-tab timer. The three Markdown navigation preferences default on and apply as one consistent snapshot. The Auto-Commit toggle is event-driven: when enabled it records only the file that successfully saved.
...
- Paste priority is internal Figaro Markdown, explicit plain-text bypass, image, protected source context, high-confidence table, semantic rich HTML, then native/plain fallback. URL-over-selection is handled earlier by CodeMirror's Markdown extension. Block conversion adds safe blank-line boundaries; inline table-cell paste never injects a block.
